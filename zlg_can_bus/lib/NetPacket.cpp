...
bool CPacketUtil::GetPacket(PacketBuffer &dequeBuffer, Packet &packet) {
  ...
  if (IsValidPacketHead(vecHead)) {
    decltype(size) nPacketLen =
        PACKET_HEAD_LEN + GetPacketDataLen(vecHead) + PACKET_CRC_LEN;
    if (size < nPacketLen) {
...
UINT64 CPacketUtil::GetNowTimeStampAsMicrosecond() {
  static std::chrono::steady_clock::time_point tp_init =
      std::chrono::steady_clock::now();
  std::chrono::steady_clock::time_point tp_now =
      std::chrono::steady_clock::now();
  UINT64 time =
      std::chrono::duration_cast<std::chrono::microseconds>(tp_now - tp_init)
          .count();
  return time;
...
int CPacketUtil::GetPacketFrameCount(const Packet &packet) {
  ...
  } else if (type == PACKET_TYPE_AUTH_REQ || type == PACKET_TYPE_AUTH_RES) {
...
bool CPacketUtil::IsPacketCompressed(const Packet &packet) {
  ...
  if (PACKET_TYPE_CAN == type || PACKET_TYPE_CANFD == type) {
    return (typeParam & PACKET_TYPE_PARAM_FLAG_COMPRESS) != 0;
  }
  return false;
}

int CPacketUtil::GetPacketDataCAN(const Packet &packet,
  ...
  if (PACKET_TYPE_CAN == GetPacketFrameType(packet)) {
    ...
    if (IsPacketCompressed(packet)) {
      ...
      int ret = uncompress((Bytef *)&packetDecompress[0], &cLen,
                           (const Bytef *)&packet[PACKET_HEAD_LEN], pktDatalen);
      if (ret != Z_OK) {
...
int CPacketUtil::GetPacketDataCANFD(const Packet &packet,
...
int CPacketUtil::GetPacketDataGPS(const Packet &packet,
...
int CPacketUtil::GetPacketDataLIN(const Packet &packet,
...
int CPacketUtil::GetPacketDataAUTH(const Packet &packet, FrameData &vecAuth,
                                   uint8_t &pktTypeParam) {
  int nRet = 0;
  uint8_t pktType = GetPacketFrameType(packet);
  if (PACKET_TYPE_AUTH_REQ == pktType || PACKET_TYPE_AUTH_RES == pktType) {
    nRet = GetPacketFrameCount(packet);
    if (nRet == 1) {
      auto itrDatabegin = packet.begin() + PACKET_HEAD_LEN;
...
int CPacketUtil::GetPacketDataBusUsage(const Packet &packet,
...
int CPacketUtil::GetPacketDataDevReq(const Packet &packet,
                                     FrameData &vecBusDevReq,
                                     uint8_t &pktTypeParam) {
  int nRet = 0;
  uint8_t pktType = GetPacketFrameType(packet);
...
int CPacketUtil::GetPacketDataDevRes(const Packet &packet,
                                     FrameData &vecBusDevRes,
                                     uint8_t &pktTypeParam) {
  int nRet = 0;
  uint8_t pktType = GetPacketFrameType(packet);
...
int CPacketUtil::GetPacketDataRequest(const Packet &packet,
  ...
  if (PACKET_TYPE_REQUEST_RESPONSE == pktType &&
      pktTypeParam.unionValue.bResponse == 0) {
    int nDataLen = GetPacketDataLen(packet);
        ...
        nDataLen + PACKET_HEAD_LEN + 1 == packet.size()) // crc 1 uint8_t
    {
      ...
      request.vecData.assign(vecData.begin() + 2,
                             vecData.begin() +
                                 nDataLen); // 2byte tid and result
      nRet = 1;
    } else {
      LOG_ERR("Packet GetPacketDataRequest Lenth Invalid, pkt.len=%d, "
              "pkt.size=%d\n",
...
int CPacketUtil::GetPacketDataRequestEx(const Packet &packet,
                                        PacketDataRequest &request) {
  int nRet = 0;
  uint8_t pktType = GetPacketFrameType(packet);
  PacketTypeRequestResponseParam pktTypeParam;
  pktTypeParam.rawValue = CPacketUtil::GetPacketFrameTypeParam(packet);
  if (PACKET_TYPE_REQUEST_RESPONSE_EX == pktType &&
      pktTypeParam.unionValue.bResponse == 0) {
    int nDataLen = GetPacketDataLen(packet);
        ...
        nDataLen + PACKET_HEAD_LEN + 1 == packet.size()) // crc 1 uint8_t
    {
      ...
      request.vecData.assign(vecData.begin() + 5,
                             vecData.begin() +
                                 nDataLen); // 5byte tid and result
      nRet = 1;
    } else {
      LOG_ERR("Packet GetPacketDataRequestEx Lenth Invalid, pkt.len=%d, "
              "pkt.size=%d\n",
              nDataLen, packet.size());
    }
  } else {
...
int CPacketUtil::GetPacketDataResponse(const Packet &packet,
  ...
  pktTypeParam.rawValue = CPacketUtil::GetPacketFrameTypeParam(packet);
  if (PACKET_TYPE_REQUEST_RESPONSE == pktType &&
      pktTypeParam.unionValue.bResponse) {
    int nDataLen = GetPacketDataLen(packet);
    if (nDataLen >= PACKET_REQUEST_RESPONSE_DATALEN_MIN &&
        nDataLen + PACKET_HEAD_LEN + 1 == packet.size()) // crc 1 uint8_t
    {
      ...
      response.vecData.assign(vecData.begin() + 2,
                              vecData.begin() +
                                  nDataLen); // 2byte tid and result
      nRet = 1;
    } else {
      LOG_ERR("Packet GetPacketDataResponse Lenth Invalid, pkt.len=%d "
              "pkt.size=%d\n",
              nDataLen, packet.size());
    }
  } else {
...
int CPacketUtil::GetPacketDataResponseEx(const Packet &packet,
                                         PacketDataResponse &response) {
  int nRet = 0;
  uint8_t pktType = GetPacketFrameType(packet);
  PacketTypeRequestResponseParam pktTypeParam;
  ...
  if (PACKET_TYPE_REQUEST_RESPONSE_EX == pktType &&
        ...
        nDataLen + PACKET_HEAD_LEN + 1 == packet.size()) // crc 1 uint8_t
    {
      ...
      response.vecData.assign(vecData.begin() + 5,
                              vecData.begin() +
                                  nDataLen); // 4byte tid and result
      nRet = 1;
    } else {
      LOG_ERR("Packet GetPacketDataResponseEx Lenth Invalid, pkt.len=%d "
              "pkt.size=%d\n",
              nDataLen, packet.size());
    }
  } else {
...
int CPacketUtil::GetPacketDataDevState(const Packet &packet,
                                       FrameData &vecDevStat,
                                       uint8_t &pktTypeParam) {
  int nRet = 0;
  uint8_t pktType = GetPacketFrameType(packet);
...
bool CPacketUtil::GetPacket(const FrameData &pktFrame, PacketDataCAN &pktCAN) {
  if (pktFrame.size() == sizeof(pktCAN)) {
    ...
    pktCAN.canHead.frameInfo.rawVal =
        cc_ntohs(pNetOrder->canHead.frameInfo.rawVal);
...
bool CPacketUtil::GetPacket(const FrameData &pktFrame,
                            PacketDataCANFD &pktCANFD) {
  if (pktFrame.size() == sizeof(pktCANFD)) {
    ...
    pktCANFD.canHead.frameInfo.rawVal =
        cc_ntohs(pNetOrder->canHead.frameInfo.rawVal);
...
bool CPacketUtil::GetPacket(const FrameData &pktFrame, PacketDataLIN &pktLIN) {
  if (pktFrame.size() == sizeof(pktLIN)) {
    ...
    pktLIN.frameInfo.linInfoRawVal =
        cc_ntohs(pNetOrder->frameInfo.linInfoRawVal);
...
bool CPacketUtil::GetPacket(const FrameData &pktFrame,
                            PacketDataAutoSend &pktAutoSend) {
...
void CPacketUtil::ZCANTx2CANFrame(const ZCAN_Transmit_Data &zTx, int nChnl,
                                  bool bTxEchoEnable, PacketDataCAN &pktFrame) {
  ...
  pktFrame.canHead.frameInfo.unionVal.bSendDelay =
      IS_DELAY_SEND(zTx.frame.__pad);
  ...
  pktFrame.canHead.nTimestamp = pktFrame.canHead.frameInfo.unionVal.bSendDelay
                                    ? nVal
                                    : GetNowTimeStampAsMicrosecond();
  ...
  // TxEcho
  pktFrame.canHead.frameInfo.unionVal.nEcho =
      bTxEchoEnable ? 1 : IS_TX_ECHO(zTx.frame.__pad);
}

void CPacketUtil::ZCANTx2CANFrame(const ZCAN_TransmitFD_Data &zTx, int nChnl,
                                  bool bTxEchoEnable,
  ...
  pktFrame.canHead.frameInfo.unionVal.bBRS =
      ((zTx.frame.flags & CANFD_BRS) != 0 ? 1 : 0);
  pktFrame.canHead.frameInfo.unionVal.bRTR = IS_RTR(zTx.frame.can_id);
  pktFrame.canHead.frameInfo.unionVal.bSendDelay =
      IS_DELAY_SEND(zTx.frame.flags);
  uint32_t nVal = MAKEWORD(zTx.frame.__res0, zTx.frame.__res1);
  ...
  // TxEcho
  pktFrame.canHead.frameInfo.unionVal.nEcho =
      bTxEchoEnable ? 1 : IS_TX_ECHO(zTx.frame.flags);
}

bool CPacketUtil::ZCANDataObj2CANFrame(const ZCANDataObj &zTx,
                                       PacketDataCANFD &pktFrame) {
  ...
  pktFrame.canHead.frameInfo.unionVal.nSendtype =
      zTx.data.zcanCANFDData.flag.unionVal.transmitType;
  pktFrame.canHead.frameInfo.unionVal.bEXT =
      IS_EFF(zTx.data.zcanCANFDData.frame.can_id);
  pktFrame.canHead.frameInfo.unionVal.bFD =
      zTx.data.zcanCANFDData.flag.unionVal.frameType;
  pktFrame.canHead.frameInfo.unionVal.bBRS =
      ((zTx.data.zcanCANFDData.frame.flags & CANFD_BRS) != 0 ? 1 : 0);
  pktFrame.canHead.frameInfo.unionVal.bRTR =
      IS_RTR(zTx.data.zcanCANFDData.frame.can_id);
  pktFrame.canHead.frameInfo.unionVal.bSendDelay =
      zTx.data.zcanCANFDData.flag.unionVal.txDelay != ZCAN_TX_DELAY_NO_DELAY;
  ...
  else // CANFD
  {
    if (zTx.data.zcanCANFDData.frame.len <= 8) {
      ...
      if (zTx.data.zcanCANFDData.frame.len % 4 != 0) {
        pktFrame.canHead.nDataLen =
            ((zTx.data.zcanCANFDData.frame.len / 4) + 1) * 4;
      } else {
        pktFrame.canHead.nDataLen = zTx.data.zcanCANFDData.frame.len;
      }
    } else {
      if (zTx.data.zcanCANFDData.frame.len % 16 != 0) {
        pktFrame.canHead.nDataLen =
            ((zTx.data.zcanCANFDData.frame.len / 16) + 1) * 16;
      } else {
        pktFrame.canHead.nDataLen = zTx.data.zcanCANFDData.frame.len;
      }
    }
  }
  memcpy(pktFrame.canData, zTx.data.zcanCANFDData.frame.data,
         sizeof(pktFrame.canData));

  // TxEcho
  pktFrame.canHead.frameInfo.unionVal.nEcho =
      zTx.data.zcanCANFDData.flag.unionVal.txEchoRequest;

  return true;
}

void CPacketUtil::CANFrame2ZCANRx(const PacketDataCAN &pktFrame,
                                  ZCAN_Receive_Data &zRx, int *pChnl) {
  ...
  zRx.frame.can_id = MAKE_CAN_ID(pktFrame.canHead.nID,
                                 pktFrame.canHead.frameInfo.unionVal.bEXT,
                                 pktFrame.canHead.frameInfo.unionVal.bRTR,
                                 pktFrame.canHead.frameInfo.unionVal.bERR);
...
void CPacketUtil::CANFrame2ZCANRx(const PacketDataCAN &pktFrame,
  ...
  canfdData.frame.can_id = MAKE_CAN_ID(
      pktFrame.canHead.nID, pktFrame.canHead.frameInfo.unionVal.bEXT,
      pktFrame.canHead.frameInfo.unionVal.bRTR,
      pktFrame.canHead.frameInfo.unionVal.bERR);
...
void CPacketUtil::CANFrame2ZCANRx(const PacketDataCANFD &pktFrame,
                                  ZCAN_ReceiveFD_Data &zRx, int *pChnl,
                                  bool bRecvMerge) {
  memset(&zRx, 0, sizeof(zRx));
  zRx.timestamp = pktFrame.canHead.nTimestamp;
...
void CPacketUtil::CANFrame2ZCANRx(const PacketDataCANFD &pktFrame,
  ...
  canfdData.frame.can_id = MAKE_CAN_ID(
      ...
      pktFrame.canHead.frameInfo.unionVal.bRTR,
      pktFrame.canHead.frameInfo.unionVal.bERR);
  canfdData.frame.flags |=
      (pktFrame.canHead.frameInfo.unionVal.bESI ? CANFD_ESI : 0);
  canfdData.frame.flags |=
      (pktFrame.canHead.frameInfo.unionVal.bBRS ? CANFD_BRS : 0);
  memcpy(canfdData.frame.data, pktFrame.canData, sizeof(canfdData.frame.data));
  canfdData.flag.unionVal.txEchoed = pktFrame.canHead.frameInfo.unionVal.nTx;
}

void CPacketUtil::CANFrame2CANFDFrame(const PacketDataCAN &pktFrameCAN,
                                      PacketDataCANFD &pktFrameCANFD) {
  memset(&pktFrameCANFD, 0, sizeof(pktFrameCANFD));
  pktFrameCANFD.canHead = pktFrameCAN.canHead;
  memcpy(pktFrameCANFD.canData, pktFrameCAN.canData,
         sizeof(pktFrameCAN.canData));
}

void CPacketUtil::CANFDFrame2CANFrame(const PacketDataCANFD &pktFrameCANFD,
                                      PacketDataCAN &pktFrameCAN) {
  memset(&pktFrameCAN, 0, sizeof(pktFrameCAN));
  pktFrameCAN.canHead = pktFrameCANFD.canHead;
  // CANFD->CAN，只拷贝8字节数据
  memcpy(pktFrameCAN.canData, pktFrameCANFD.canData,
         sizeof(pktFrameCAN.canData));
}

bool CPacketUtil::CANErrorData2ZcanErrInfo(const PacketDataCANHead &canHead,
                                           const uint8_t *data,
                                           ZCAN_CHANNEL_ERR_INFO &rZCANFrame) {
...
bool CPacketUtil::CANErrorData2ZCANDataObj(const PacketDataCANHead &canHead,
                                           const uint8_t *data,
                                           ZCANDataObj &zcanDataObj) {
  if (!canHead.frameInfo.unionVal.bERR || nullptr == data) {
    return false;
  }
  ...
  case 0xE1: {
    //总线错误,
    // nBusErrSubcode表示具体错误类型，节点状态有效，根据Rx，tx错误计数推断
    ...
    errData.nodeState =
        CPacketUtil::GetNodeStateByTxRxErrCounter(nTxErr, nRxErr);
  ...
  case 0xE6: {
    ...
    case 0x05:
      ...
      break;
    default:
      LOG_ERR("ErrCode:0xE6, Controler Error, unkown sub code:0x%02X , will be "
              "ignored\n",
              nErrData);
      break;
    }
  ...
  case 0xE7:
    ...
    case 0x02:
      ...
      break;
    default:
      LOG_ERR("ErrCode:0xE7, Device Error, unkown sub code:0x%02X , will be "
              "ignored\n",
              nErrData);
      break;
    }
  ...
  if (0 == errData.errType) {
    LOG_ERR("Error Frame Generate invalid ErrData.errType = 0, "
            "data[0...2]=0x%02X 0x%02X 0x%02X RxErr:%d TxErr:%d\n",
...
void CPacketUtil::PacketDataBusUsage2ZCANDataObj(
    const PacketDataBusUsage &pktBusUsage, ZCANDataObj &zRx) {
  memset(&zRx, 0, sizeof(zRx));
...
VecVecZCANDataObjPtr CPacketUtil::SplitZCANData(const ZCANDataObj *pSend,
                                                ULONG len) {
...
bool CPacketUtil::PacketData2ZCANDataObj(const PacketDataCANFD &pktFrame,
                                         ZCANDataObj &zCANObj) {
  if (pktFrame.canHead.frameInfo.unionVal.bERR) {
    return CANErrorData2ZCANDataObj(pktFrame.canHead, pktFrame.canData,
                                    zCANObj);
  } else {
...
bool CPacketUtil::PacketData2ZCANDataObj(const PacketDataCAN &pktFrame,
                                         ZCANDataObj &zCANObj) {
  if (pktFrame.canHead.frameInfo.unionVal.bERR) {
    return CANErrorData2ZCANDataObj(pktFrame.canHead, pktFrame.canData,
                                    zCANObj);
...
bool CPacketUtil::PacketData2ZCANDataObj(const PacketDataGPS &pktFrame,
                                         ZCANDataObj &zCANObj) {
  PacketGPS2ZCANDataObj(pktFrame, zCANObj);
  return true;
}

bool CPacketUtil::PacketData2ZCANDataObj(const PacketDataLIN &pktFrame,
                                         ZCANDataObj &zCANObj) {
  PacketLIN2ZCANDataObj(pktFrame, zCANObj);
  return true;
}

bool CPacketUtil::PacketData2ZCANDataObj(const PacketDataBusUsage &pktBusUsage,
                                         ZCANDataObj &zCANObj) {
  PacketDataBusUsage2ZCANDataObj(pktBusUsage, zCANObj);
  return true;
}

bool CPacketUtil::CANFrame2ErrFrame(const PacketDataCAN &pktFrame,
                                    ZCAN_CHANNEL_ERR_INFO &errInfo) {
  return CANErrorData2ZcanErrInfo(pktFrame.canHead, pktFrame.canData, errInfo);
}

bool CPacketUtil::CANFrame2ErrFrame(const PacketDataCANFD &pktFrame,
                                    ZCAN_CHANNEL_ERR_INFO &errInfo) {
  return CANErrorData2ZcanErrInfo(pktFrame.canHead, pktFrame.canData, errInfo);
}

void CPacketUtil::ZCANAutoSend2AutoSendFrame(const ZCAN_AUTO_TRANSMIT_OBJ &zCAN,
                                             int nChnl, bool bTxEchoEnable,
...
void CPacketUtil::ZCANAutoSend2AutoSendFrame(
    const ZCANFD_AUTO_TRANSMIT_OBJ &zCAN, int nChnl, bool bTxEchoEnable,
    PacketDataAutoSend &autoSend) {
  memset(&autoSend, 0, sizeof(autoSend));
  autoSend.nIndex = (uint8_t)zCAN.index;
  autoSend.bEnable = (uint8_t)zCAN.enable;
...
void CPacketUtil::AutoSendData2AutoSendFrame(
    const AutoSendData &autoSendData, int nChnl, bool bTxEchoEnable,
    PacketDataAutoSend &autoSendFrame) {
  ...
  if (autoSendData.iscanfd) {
    ZCANTx2CANFrame(autoSendData.data.obj, nChnl, bTxEchoEnable,
                    autoSendFrame.pktData);
  } else {
...
void CPacketUtil::AutoSendFrame2ZCANAutoSend(const PacketDataAutoSend &autoSend,
  ...
  zCAN.obj.frame.can_id =
      MAKE_CAN_ID(autoSend.pktData.canHead.nID,
                  autoSend.pktData.canHead.frameInfo.unionVal.bEXT,
                  autoSend.pktData.canHead.frameInfo.unionVal.bRTR,
                  autoSend.pktData.canHead.frameInfo.unionVal.bERR);
  memcpy(zCAN.obj.frame.data, autoSend.pktData.canData, CAN_MAX_DLEN);
  nChnl = autoSend.pktData.canHead.nChnl;
}

void CPacketUtil::AutoSendFrame2ZCANAutoSend(const PacketDataAutoSend &autoSend,
                                             ZCANFD_AUTO_TRANSMIT_OBJ &zCAN,
                                             int &nChnl) {
  ...
  zCAN.obj.frame.flags |=
      (autoSend.pktData.canHead.frameInfo.unionVal.bBRS ? CANFD_BRS : 0);
  zCAN.obj.frame.flags |=
      (autoSend.pktData.canHead.frameInfo.unionVal.bESI ? CANFD_ESI : 0);
  zCAN.obj.frame.can_id =
      MAKE_CAN_ID(autoSend.pktData.canHead.nID,
                  autoSend.pktData.canHead.frameInfo.unionVal.bEXT,
                  autoSend.pktData.canHead.frameInfo.unionVal.bRTR,
...
void CPacketUtil::PacketGPS2ZGPS(const PacketDataGPS &pktGPS,
                                 ZCAN_GPS_FRAME &zGPS) {
...
void CPacketUtil::PacketGPS2ZCANDataObj(const PacketDataGPS &pktGPS,
                                        ZCANDataObj &zRx) {
  memset(&zRx, 0, sizeof(zRx));
...
void CPacketUtil::PacketLIN2ZLIN(const PacketDataLIN &pktLIN,
                                 ZCAN_LIN_MSG &zLINMsg, int *pChnl) {
  ...
  } else //错误帧
  {
    ...
    zLINErrData.errData.errStage =
        pktLIN.frameInfo.linInfo.linRxInfo.unionLinRx.bErrStage;
    zLINErrData.errData.errReason =
        pktLIN.frameInfo.linInfo.linRxInfo.unionLinRx.bErrReason;
  }

  return;
}

void CPacketUtil::PacketLIN2ZCANDataObj(const PacketDataLIN &rProtocalFrame,
                                        ZCANDataObj &rZCANDataObj) {
  ...
  if (!rProtocalFrame.frameInfo.linInfo.linRxInfo.unionLinRx.bERR) {
    ...
    zLIN.RxData.dir =
        rProtocalFrame.frameInfo.linInfo.linRxInfo.unionLinRx.bDIR;
  ...
  } else //错误帧
  {
    ...
    zLINErrData.errData.errStage =
        rProtocalFrame.frameInfo.linInfo.linRxInfo.unionLinRx.bErrStage;
    zLINErrData.errData.errReason =
        rProtocalFrame.frameInfo.linInfo.linRxInfo.unionLinRx.bErrReason;
  }
}

void CPacketUtil::ZLIN2PacketLIN(int nChnl, const ZCAN_LIN_MSG &zLIN,
                                 PacketDataLIN &pktLIN) {
  // memset(&pktLIN, 0, sizeof(pktLIN));
  // pktLIN.nChnl = nChnl;
...
bool CPacketUtil::ZCANDataObj2PacketLIN(const ZCANDataObj &zTx,
                                        PacketDataLIN &pktLIN) {
...
//发送头
void CPacketUtil::ZLIN2PacketLIN(int nChnl, const ZCANLINData &zcanLINData,
                                 PacketDataLIN &pktLIN) {
...
//设置发布数据
void CPacketUtil::ZLIN2PacketLIN(int nChnl,
                                 const ZCAN_LIN_PUBLISH_CFG &zLINResponse,
                                 PacketDataLIN &pktLIN) {
  if (zLINResponse.chkSumMode >= AUTOMATIC ||
      zLINResponse.ID > MAX_LIN_ID_COUNT ||
  ...
  pktLIN.frameInfo.linInfo.linTxInfo.unionLinTx.bChkSumMode =
      zLINResponse.chkSumMode;
...
//设置订阅
void CPacketUtil::ZLIN2PacketLIN(int nChnl,
                                 const ZCAN_LIN_SUBSCIBE_CFG &zLINSubscibe,
                                 PacketDataLIN &pktLIN) {
  if (zLINSubscibe.chkSumMode > AUTOMATIC ||
      zLINSubscibe.ID > MAX_LIN_ID_COUNT || zLINSubscibe.dataLen == 0) {
    return;
  }
  if (zLINSubscibe.dataLen > MAX_LIN_DLC &&
      zLINSubscibe.dataLen < AUTO_LIN_DLC) {
    return;
  }
  memset(&pktLIN, 0, sizeof(pktLIN));
  pktLIN.nChnl = nChnl;
...
VecPackets CPacketEncoder::BuildPacketCAN(int nChnl,
                                          const ZCAN_Transmit_Data *pSend,
                                          ULONG len, bool bTxEchoEnable) {
  ...
  while (len) {
    ...
    for (ULONG i = 0; i < cnt; i++) {
      CPacketUtil::ZCANTx2CANFrame(pSend[doneCnt + i], nChnl, bTxEchoEnable,
                                   pktFrame);
...
VecPackets CPacketEncoder::BuildPacketCANFD(int nChnl,
                                            const ZCAN_TransmitFD_Data *pSend,
                                            ULONG len, bool bTxEchoEnable) {
  VecPackets vecPackets;
  Packet packet;
  PacketDataCANFD pktFrame;
  const ULONG max_frame_cnt =
      18; // max 1460,  1460 - 6 - 1 / FRAME_LEN_CANFD = 18
  ULONG doneCnt = 0;
  while (len) {
    ULONG cnt = len < max_frame_cnt ? len : max_frame_cnt;
    packet.clear();
...
VecPackets
CPacketEncoder::BuildPacketCANCANFD(const VecZCANDataObjPtr &vecData) {
  VecPackets vecPackets;
  Packet packet;
  PacketDataCANFD pktFrame;
  const ULONG max_frame_cnt =
...
VecPackets CPacketEncoder::BuildPacketMerge(const ZCANDataObj *pSend,
                                            ULONG len) {
...
Packet CPacketEncoder::BuildPacketAUTH(bool bRequest, const uint8_t *data,
                                       unsigned int len, uint8_t autoIncIndex) {
  Packet packet;
  if (nullptr != data && len == FRAME_LEN_AUTH) {
    PacketAddHead(packet, BuildPacketHead((bRequest ? PACKET_TYPE_AUTH_REQ
                                                    : PACKET_TYPE_AUTH_RES),
                                          1, autoIncIndex));
...
Packet CPacketEncoder::BuildPacketAutosend(int nChnl,
                                           const ZCAN_AUTO_TRANSMIT_OBJ *pObj,
                                           bool bTxEchoEnable) {
  Packet packet;
  PacketAddHead(packet, BuildPacketHead(PACKET_TYPE_CAN_AUTO_SEND, 1, 0));
  PacketDataAutoSend autoSend;
  CPacketUtil::ZCANAutoSend2AutoSendFrame(*pObj, nChnl, bTxEchoEnable,
                                          autoSend);
...
Packet CPacketEncoder::BuildPacketAutosend(int nChnl,
                                           const ZCANFD_AUTO_TRANSMIT_OBJ *pObj,
                                           bool bTxEchoEnable) {
  Packet packet;
  PacketAddHead(packet, BuildPacketHead(PACKET_TYPE_CAN_AUTO_SEND, 1, 0));
  PacketDataAutoSend autoSend;
...
VecPackets CPacketEncoder::BuildPacketAutosend(
    int nChnl, const VecAutoSendData &vecAutoSend, bool bTxEchoEnable) {
  VecPackets vecPackets;
  Packet packet;
  ...
  while (len) {
    ULONG cnt = len < max_frame_cnt ? len : max_frame_cnt;
    packet.clear();
    PacketAddHead(packet,
                  BuildPacketHead(PACKET_TYPE_CAN_AUTO_SEND, (int)cnt, 0));
    for (ULONG i = 0; i < cnt; i++) {
      CPacketUtil::AutoSendData2AutoSendFrame(vecAutoSend[doneCnt + i], nChnl,
                                              bTxEchoEnable, pktFrame);
      PacketAddData(packet, pktFrame);
    }
    PacketAddCRC(packet);
    vecPackets.push_back(packet);
...
Packet CPacketEncoder::BuildPacketDevReqRes(bool bRequest, const uint8_t *data,
                                            unsigned int len,
                                            uint8_t pktTypeParam) {
  Packet packet;
  uint8_t pktType = bRequest ? PACKET_TYPE_DEV_REQ : PACKET_TYPE_DEV_RES;
  PacketHead head{PACKET_HEADER, pktType, pktTypeParam, 0,
...
Packet CPacketEncoder::BuildPacketRequest(uint8_t nSeq,
                                          const PacketDataRequest &request) {
  Packet packet;
  ...
  PacketHead head{PACKET_HEADER, PACKET_TYPE_REQUEST_RESPONSE,
                  pktParam.rawValue, 0, len};
  PacketAddHead(packet, head);
  uint8_t tmp[2];
  tmp[0] = request.nTID;
...
Packet CPacketEncoder::BuildPacketRequestEx(uint8_t nSeq,
                                            const PacketDataRequest &request) {
  Packet packet;
  PacketTypeRequestResponseParam pktParam;
  pktParam.rawValue = 0;
  ...
  PacketHead head{PACKET_HEADER, PACKET_TYPE_REQUEST_RESPONSE_EX,
                  pktParam.rawValue, 0, len};
  PacketAddHead(packet, head);
  uint8_t tmp[5];
...
Packet CPacketEncoder::BuildPacketEmulateResponse(
    uint8_t nSeq, const PacketDataRequest &requestData) {
  Packet packet;
  PacketTypeRequestResponseParam pktParam;
  pktParam.rawValue = 0;
  ...
  PacketAddData(packet, tmp, sizeof(tmp));
  PacketAddData(packet, responseData.vecData.data(),
                responseData.vecData.size());
  PacketAddCRC(packet);
  return packet;
}

Packet CPacketEncoder::BuildPacketEmulateResponseEx(
    uint8_t nSeq, const PacketDataRequest &requestData) {
  Packet packet;
  PacketTypeRequestResponseParam pktParam;
  pktParam.rawValue = 0;
...
void CPacketEncoder::GenRequestResponseEmulateData(
    const PacketDataRequest &requestData, PacketDataResponse &responseData) {
  ...
  } else if (requestData.nRequest ==
             PACKET_REQUEST_GET_DEVICE_DELAY_SEND_STATE) {
    char devInfo[200] = {0};
    int nLen = snprintf(devInfo, sizeof(devInfo),
                        "{\r\n\"Size\":%d,\r\n\"Remain\":%d\r\n}", 0, 0);
    uint8_t *itr = (uint8_t *)&devInfo[0];
...
VecPackets CPacketEncoder::BuildPacketLIN(int nChnl, const PZCAN_LIN_MSG pSend,
                                          ULONG len) {
  VecPackets vecPackets;
  Packet packet;
  ...
  while (len) {
    ...
    for (ULONG i = 0; i < cnt; i++) {
      CPacketUtil::ZLIN2PacketLIN(nChnl, pSend[doneCnt + i].data.zcanLINData,
                                  pktFrame);
      PacketAddData(packet, pktFrame);
    }
    PacketAddCRC(packet);
...
VecPackets CPacketEncoder::BuildPacketLIN(int nChnl,
                                          const PZCAN_LIN_SUBSCIBE_CFG pSend,
                                          ULONG len) {
  VecPackets vecPackets;
  Packet packet;
  PacketDataLIN pktFrame;
...
VecPackets CPacketEncoder::BuildPacketLIN(int nChnl,
                                          const PZCAN_LIN_PUBLISH_CFG pSend,
                                          ULONG len) {
  VecPackets vecPackets;
  Packet packet;
  PacketDataLIN pktFrame;
...
Packet
CPacketEncoder::BuildPacketEchoResponse(const PacketDataCANFD &canFrame) {
  Packet packet;
...
PacketHead CPacketEncoder::BuildPacketHead(uint8_t pktType, int pktCnt,
                                           uint8_t pktTypeParam /*= 0*/) {
...
void CPacketEncoder::PacketAddData(Packet &packet,
                                   const PacketDataCAN &canFrame) {
  ...
  netFrame.canHead.frameInfo.rawVal =
      cc_htons(canFrame.canHead.frameInfo.rawVal);
...
void CPacketEncoder::PacketAddData(Packet &packet,
                                   const PacketDataCANFD &canFDFrame) {
  ...
  netFrame.canHead.frameInfo.rawVal =
      cc_htons(canFDFrame.canHead.frameInfo.rawVal);
...
void CPacketEncoder::PacketAddData(Packet &packet,
                                   const PacketDataAutoSend &autoSendFrame) {
...
void CPacketEncoder::PacketAddData(Packet &packet, const uint8_t *data,
                                   unsigned int len) {
