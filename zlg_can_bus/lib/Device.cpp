...
USHORT UtilVersionString2USHORT(const char *version) {
  ...
  if (nIndex != std::string::npos) {
    ...
    if (nIndex1 != std::string::npos) {
      strArray[1].assign(strVersion.begin() + nIndex + 1,
                         strVersion.begin() + nIndex1);
...
static struct DeviceInfo {
  ...
  UINT
      devTxEcho; //设备是否支持发送回显，ZCANPRO中支持发送回显的会同时使用通道合并功能
  const char *devName;
} s_devinfo[] = {
...
Device::Device(UINT devType, UINT devIdx, UINT Reserved /*= 0*/)
    : m_devType(devType), m_devIdx(devIdx), m_srcPort(Reserved),
      m_frmMerge(CAN_DATA_QUEUE_BUFF_SIZE * 2) {
  ...
  for (size_t i = 0; i < sizeof(s_devinfo) / sizeof(s_devinfo[0]); i++) {
    if (devType == s_devinfo[i].devType) {
      ...
      memcpy(m_stDevInfo.str_hw_Type, m_devName.c_str(),
             min(m_devName.length(), (size_t)40));

      m_bRecvMerge = false;

#ifdef WIN32
      LOG_ALWAYS("tid = %u devtname:%s chnl:%d TCP:%d RecvMerge:%d\n",
                 GetCurrentThreadId(), m_devName.c_str(), m_devChnlCount,
                 m_devTCP, m_bRecvMerge);
#endif
...
bool Device::GetReference(UINT chnIdx, UINT refType, void *pData) {
  ...
  case CMD_CLIENT: {
    ...
    if (!pRClient || pRClient->iIndex < 0 || !svr ||
        pRClient->iIndex >= (int)svr->GetClients()->size()) {
      return false;
    }
  ...
  case GETREF_GET_DEV_STATE_SYS_INFO: {
    if (pData != NULL) {
      ...
      if (GetDevState(PACKET_TYPE_DEV_STATE_PARAM_SYS_INFO, strResult)) {
        ...
        memcpy(pVal->pData, strResult.c_str(),
               min(pVal->nResultLen, pVal->nDataLen));
        return true;
      }
    }
    return false;
...
bool Device::SetReference(UINT chnIdx, UINT refType, void *pData) {
  ...
  case SETREF_ADD_TIMER_SEND_CAN_DIRECT: {
    if (chnIdx < m_devChnlCount && pData != nullptr) {
      ZCAN_AUTO_TRANSMIT_OBJ *pObj =
          reinterpret_cast<ZCAN_AUTO_TRANSMIT_OBJ *>(pData);
      if (pObj) {
  ...
  case SETREF_ADD_TIMER_SEND_CANFD_DIRECT: {
    if (chnIdx < m_devChnlCount && pData != nullptr) {
      ZCANFD_AUTO_TRANSMIT_OBJ *pObj =
          reinterpret_cast<ZCANFD_AUTO_TRANSMIT_OBJ *>(pData);
      if (pObj) {
        return SetAutoSend(chnIdx, pObj);
      }
    }
  ...
  case SETREF_ADD_DYNAMIC_BY_JSON: {
    if (pData != NULL) {
      ZCAN_DYNAMIC_CONFIG_DATA *pObj =
          reinterpret_cast<ZCAN_DYNAMIC_CONFIG_DATA *>(pData);
      if (pObj) {
  ...
  case SETREF_ADD_FILTER: {
    if (chnIdx < m_devChnlCount && pData != nullptr) {
      CANFDNETFilterItem *pFilterItem =
          reinterpret_cast<CANFDNETFilterItem *>(pData);
  ...
  case SETREF_SET_DYNAMIC_BY_DATA: {
    if (chnIdx < m_devChnlCount && pData != nullptr) {
      ZCAN_DYNAMIC_CONFIG *pObj =
          reinterpret_cast<ZCAN_DYNAMIC_CONFIG *>(pData);
      if (pObj) {
...
void Device::ClearBuffer(UINT chnIdx) {
  if (chnIdx < m_devChnlCount) {
    LOG_INFO("ClearBuffer Chnl %d: CAN:%d CANFD:%d Err:%d\n", chnIdx,
             m_frmCAN[chnIdx].size(), m_frmCANFD[chnIdx].size(),
             m_errQueue[chnIdx].size());
...
ULONG Device::Transmit(UINT chnIdx, const ZCAN_Transmit_Data *pSend,
                       ULONG len) {
  ULONG sentCnt = 0;
...
ULONG Device::TransmitFD(UINT chnIdx, const ZCAN_TransmitFD_Data *pSend,
                         ULONG len) {
  ULONG sentCnt = 0;
  VecPackets vecPackets;
  if (chnIdx >= m_devChnlCount || pSend == nullptr || len == 0) {
...
ULONG Device::Receive(UINT chnIdx, ZCAN_Receive_Data *pReceive, ULONG len,
                      int waitTime /* = -1*/) {
...
ULONG Device::ReceiveFD(UINT chnIdx, ZCAN_ReceiveFD_Data *pReceive, ULONG len,
                        int waitTime /* = -1*/) {
  if (chnIdx >= m_devChnlCount || pReceive == nullptr || len == 0) {
    return 0;
  }
...
ULONG Device::ReceiveData(ZCANDataObj *pReceive, ULONG len,
                          int waitTime /*= -1*/) {
...
bool Device::SetAutoSend(UINT chnIdx, VecAutoSendData &vecAutoSend) {
  ...
  if (vecAutoSend.size() > 0) {
    VecPackets vecPackets;
    vecPackets =
        CPacketEncoder::BuildPacketAutosend(chnIdx, vecAutoSend, m_bTxEcho);
    for (auto &packet : vecPackets) {
      if (packet.size() != SendData(packet.data(), packet.size())) {
        return false;
      }
...
unsigned int Device::SendData(const BYTE *data, unsigned int len) {
  ...
  if (nullptr == data || len == 0 || !m_service ||
...
bool Device::VerifyDeviceByPass(VerifyDeviceData &data) {
  ...
  Packet packet = CPacketEncoder::BuildPacketAUTH(
      true, reinterpret_cast<const BYTE *>(data.inData), sizeof(data.inData),
      session);
...
bool Device::StartCAN(UINT chnIdx) {
  ...
  if (CreateAndStartService()) {
    ...
    if (m_devType == ZCAN_CANFDNET_TCP || m_devType == ZCAN_CANFDNET_UDP ||
        m_devType == ZCAN_CANFDNET_400U_TCP ||
        m_devType == ZCAN_CANFDNET_400U_UDP ||
        m_devType == ZCAN_CANFDNET_100U_TCP ||
        m_devType == ZCAN_CANFDNET_100U_UDP ||
        m_devType == ZCAN_CANFDNET_800U_TCP ||
        m_devType == ZCAN_CANFDNET_800U_UDP) {
      SyncDevClock(); //同步设备时间
    }
  ...
  if (!m_service) {
    ...
    if (m_devTCP) {
      if (m_workMode == TCP_SERVER) {
        ...
        // int *)&std::this_thread::get_id(), m_srcPort);
        m_service = new TcpServer(m_srcPort, DEV_MAX_CLIENTS);
      } else {
        // tcp client
        // LOG_ALWAYS("tid = %u, Tcp client mode. connect to:%s:%d\n",
        // *(unsigned int *)&std::this_thread::get_id(), m_destIp.c_str(),
        // m_destPort);
        m_service = new TcpClient(m_destIp.c_str(), m_destPort, m_devType);
      }
    } else {
      // LOG_ALWAYS("tid = %u, UDP mode. %d to:%s:%d\n", *(unsigned int
      // *)&std::this_thread::get_id(), m_srcPort, m_destIp.c_str(),
      // m_destPort);
      m_service = new UdpService(m_srcPort, m_destIp.c_str(), m_destPort,
                                 m_udpmulticastloopback);
    }
#else
    {
      LOG_ALWAYS("UDP mode. %d to:%s:%d\n", m_srcPort, m_destIp.c_str(),
                 m_destPort);
      m_service = new UdpService(m_srcPort, m_destIp.c_str(), m_destPort,
                                 m_udpmulticastloopback);
    }
    ...
    m_service->SetRecvCallback(
        std::bind(&Device::DealReceivedData, this, std::placeholders::_1,
...
bool Device::_StopAndClear() {
  ...
  double time = ddt / ddf;
  // std::cout << __FUNCTION__ << "^^^^ StopCAN time is: " << time * 1000 <<
  // "ms" << std::endl;
  // LOG_ALWAYS("%s^^^^ DisconnectServer time is: %fms", __FUNCTION__, time *
  // 1000);
#endif
  ...
  // m_statistic.nTxBytesSuccess, m_statistic.nTxBytesFailed,
  // m_statistic.nRxBytes);
  ...
  for (size_t i = 0; i < m_devChnlCount; i++) {
    ...
    // log chnl statistic
    /*
    LOG_ALWAYS("Chnl %d: TxCAN:%I64u(S:%I64u) TxCANFD:%I64u(S:%I64u)
    RxCAN:%I64u(D:%I64u) RxCANFD:%I64u(D:%I64u) Err:%I64u TxEcho:%I64u
    RxMerge:%I64u(D:%I64u) AppRxMerge:%I64u", i, m_statistic.chnl[i].nTxCAN,
    m_statistic.chnl[i].nTxCANSim, m_statistic.chnl[i].nTxCANFD,
    m_statistic.chnl[i].nTxCANFDSim, m_statistic.chnl[i].nRxCAN,
    m_statistic.chnl[i].nRxCANDrop, m_statistic.chnl[i].nRxCANFD,
    m_statistic.chnl[i].nRxCANFDDrop, m_statistic.chnl[i].nError,
    m_statistic.chnl[i].nTxEcho, m_statistic.chnl[i].nRxMerge,
    m_statistic.chnl[i].nRxMergeDrop, m_statistic.chnl[i].nAppRxMerge);
*/
  }
...
bool Device::InitLIN(UINT chnIdx, PZCAN_LIN_INIT_CONFIG pLINInitConfig) {
  if (chnIdx < DEV_LIN_COUNT_MAX && pLINInitConfig != NULL) {
    ...
    if (pLINInitConfig->chkSumMode == AUTOMATIC) {
      m_linConfig[chnIdx].stFeature.unionVal.bEnhancedChksum = 1; // 2;
    }
    // m_linConfig[chnIdx].stFeature.unionVal.bVarDLCDisabled =
    // !(pLINInitConfig->linFlag & LIN_FLAG_VAR_DLC);
    m_linConfig[chnIdx].nBaud = pLINInitConfig->linBaud;
...
UINT Device::ReceiveLIN(UINT chnIdx, PZCAN_LIN_MSG pReceive, UINT Len,
                        int WaitTime) {
...
UINT Device::SetLINSubscribe(UINT chnIdx, PZCAN_LIN_SUBSCIBE_CFG pSend,
                             UINT nMsgCount) {
  UINT sentCnt = 0;
  VecPackets vecPackets;
...
UINT Device::SetLINPublish(UINT chnIdx, PZCAN_LIN_PUBLISH_CFG pSend,
                           UINT nMsgCount) {
  UINT sentCnt = 0;
  VecPackets vecPackets;
  if (chnIdx >= m_devLINCount || pSend == nullptr || nMsgCount == 0) {
...
bool Device::AddAutoSend(UINT chnIdx, const ZCAN_AUTO_TRANSMIT_OBJ *pObj) {
  if (chnIdx < m_devChnlCount && pObj &&
      pObj->index < DEV_AUTO_SEND_INDEX_MAX) {
...
AutoSendData *Device::GetAutoSendIndex(VecAutoSendData &vecAutoSend,
                                       UINT autoSendIndex) {
  for (auto &item : vecAutoSend) {
...
void Device::ZCANAutoSend2AutoSend(const ZCAN_AUTO_TRANSMIT_OBJ *pObj,
                                   AutoSendData &autoSend) {
...
void Device::ZCANAutoSend2AutoSend(const ZCANFD_AUTO_TRANSMIT_OBJ *pObj,
                                   AutoSendData &autoSend) {
  memset(&autoSend, 0, sizeof(autoSend));
  autoSend.iscanfd = 1;
  autoSend.sendcount = 0;
...
bool Device::IsDevStartLessThan(long long ms) {
  std::chrono::steady_clock::time_point tp_recv =
      std::chrono::steady_clock::now();
  std::chrono::milliseconds time_span =
      std::chrono::duration_cast<std::chrono::milliseconds>(
          tp_recv - m_tpDevStartTickCount);
...
bool Device::SetDevTimeStamp(UINT64 nTimeStampUs) {
  ...
  Packet packet = CPacketEncoder::BuildPacketDevReqRes(
      true, data, sizeof(nTimeNetOrder), reqParam.rawValue);
  return SendData(packet.data(), packet.size()) == packet.size();
}

UINT Device::GetTxEchoTimeStamp(UINT chnIdx, TxTimeStamp &rTxTimeStamp) {
  if (chnIdx >= m_devChnlCount || rTxTimeStamp.pTxTimeStampBuffer == nullptr ||
      rTxTimeStamp.nBufferTimeStampCount == 0) {
    return 0;
  }
...
bool Device::GetDelaySendAvailableTxCount(UINT chnIdx, UINT &nCount) {
  if (chnIdx < m_devChnlCount) {
    ...
    if (GetDelaySendInfo(chnIdx, strDelayInfo)) {
      ...
      if (pInfo) {
        UINT nSize = (UINT)UtilGetcJSONNumber(
            cJSON_DetachItemFromObjectCaseSensitive(pInfo, "Size"));
        nCount = (UINT)UtilGetcJSONNumber(
            cJSON_DetachItemFromObjectCaseSensitive(pInfo, "Remain"));
        cJSON_Delete(pInfo);
        return true;
      }
    }
...
bool Device::GetDevAutoSendListCount(UINT chnIdx, bool bCANFD, UINT &nCount) {
  if (chnIdx < m_devChnlCount) {
    std::chrono::steady_clock::time_point tp_now =
        std::chrono::steady_clock::now();
    std::chrono::milliseconds time_span =
        std::chrono::duration_cast<std::chrono::milliseconds>(
            tp_now - m_devAutoSendList.chnl[chnIdx].m_tpRecv);
    if (time_span.count() > REQUEST_TIMEOUT) {
      ...
      m_devAutoSendList.chnl[chnIdx].vecCAN.clear();
      m_devAutoSendList.chnl[chnIdx].vecCANFD.clear();
      m_devAutoSendList.chnl[chnIdx].m_tpRecv =
          std::chrono::steady_clock::now();
...
bool Device::RequestDevData(PacketDataRequest &devRequest,
                            PacketDataResponse &devResponse,
                            bool autoProtocol) {
  bool bRet = false;
  ...
  if (packet.size() == SendData(packet.data(), packet.size())) {
    std::mutex mutex_;
    std::unique_lock<std::mutex> ulk(mutex_);
    if (data->cv.wait_for(ulk, std::chrono::milliseconds(REQUEST_TIMEOUT),
                          [&data] { return data->responsed; })) {
...
void Device::GenGetAutoSendListRequest(UINT chnIdx, UINT nOffset,
                                       UINT nRequestCount,
                                       PacketDataRequest &devRequest) {
  ...
  int nLen = snprintf(requestJson, 200,
                      "{\r\n\"Chn\":%d,\r\n\"Offset\":%d,\r\n\"Num\":%d\r\n}",
                      chnIdx, nOffset, nRequestCount) +
             1;
  BYTE *pData = (BYTE *)&requestJson[0];
  devRequest.vecData.assign(pData, pData + nLen);
}

void Device::GenGetDevDelaySendRequest(UINT chnIdx,
                                       PacketDataRequest &devRequest) {
...
void Device::GenClearDelaySendRequest(UINT chnIdx,
                                      PacketDataRequest &devRequest) {
...
bool Device::GetDevAutoSendList(UINT chnIdx,
                                std::vector<PacketDataAutoSend> &vecAutoSend) {
  //由于帧长度限制，一帧定时发送占用88字节，一个以太网帧可以包装(1460-6-1)/88 =
  // 16 帧
  ...
  while (nReaded < nTotalCount) {
    ...
    if (RequestDevData(request, response)) {
      if (response.nResult) {
        ...
        std::string strResult(response.vecData.begin(), response.vecData.end());
        UINT nCount = GetAutoSendListFromJsonStr(chnIdx, nReaded, nReadOnce,
                                                 strResult, vecAutoSendOnce);
        vecAutoSend.insert(vecAutoSend.end(), vecAutoSendOnce.begin(),
                           vecAutoSendOnce.end());
        nReaded += nCount;
...
bool Device::GetDevInfoFromJsonStr(const std::string &strDevInfo,
  ...
  if (pDevInfo) {
    std::string strModel =
        UtilGetcJSONString(cJSON_GetObjectItemCaseSensitive(pDevInfo, "Model"));
    std::string strFirmware = UtilGetcJSONString(
        cJSON_GetObjectItemCaseSensitive(pDevInfo, "Firmware"));
    std::string strHardware = UtilGetcJSONString(
        cJSON_GetObjectItemCaseSensitive(pDevInfo, "Hardware"));
    std::string strSN =
        UtilGetcJSONString(cJSON_GetObjectItemCaseSensitive(pDevInfo, "SN"));

    // 20201105，通讯协议版本，V1.10后存在此字段
    std::string protoVer = UtilGetcJSONString(
        cJSON_GetObjectItemCaseSensitive(pDevInfo, "ProtoVer"));
    memset(m_protocolVersion, 0, sizeof(m_protocolVersion));
    ...
    memcpy(pInfo->str_hw_Type, strModel.c_str(),
           min(strModel.length(), sizeof(pInfo->str_hw_Type)));
    memcpy(pInfo->str_Serial_Num, strSN.c_str(),
           min(strSN.length(), sizeof(pInfo->str_Serial_Num)));
    ...
    UINT nCanChnNum = (UINT)UtilGetcJSONNumber(
        cJSON_GetObjectItemCaseSensitive(pDevInfo, "CanChnNum"));
    UINT nLinChnNum = (UINT)UtilGetcJSONNumber(
        cJSON_GetObjectItemCaseSensitive(pDevInfo, "LinChnNum"));
...
UINT Device::GetAutoSendListFromJsonStr(
    UINT chnIdx, UINT nOffset, UINT nRequestCount,
    const std::string &strAutoSendJson,
    std::vector<PacketDataAutoSend> &vecAutoSend) {
  ...
  if (pAutoSendMsgs) {
    ...
    for (UINT i = nOffset; i < nOffset + nRequestCount; ++i) {
      ...
      cJSON *pSendItem =
          cJSON_GetObjectItemCaseSensitive(pAutoSendMsgs, key.c_str());
      if (pSendItem) {
...
void Device::GetAutoSendFromJson(UINT nChnIdx, UINT nIndex, cJSON *config,
  ...
  // 20201106
  // 协议版本更新V1.10之后添加发送周期单位字段，无参数或者参数值为0表示单位是ms，值为1表示时间单位为100us(0.1ms)
  int nPeriodUnit = (int)UtilGetcJSONNumber(
      cJSON_GetObjectItemCaseSensitive(config, "PeriodUnit"));
  nPeriodUnit = !!nPeriodUnit;
  int nPeriodValue = (int)UtilGetcJSONNumber(
      cJSON_GetObjectItemCaseSensitive(config, "Period"));
  ...
  autoSend.bEnable = (BYTE)UtilGetcJSONNumber(
      cJSON_GetObjectItemCaseSensitive(config, "Enable"));
  autoSend.nRepeatCount = (USHORT)UtilGetcJSONNumber(
      cJSON_GetObjectItemCaseSensitive(config, "Cnt"));

  autoSend.pktData.canHead.nID = (int)UtilGetcJSONNumber(
      cJSON_GetObjectItemCaseSensitive(config, "MsgID"));
  autoSend.pktData.canHead.frameInfo.rawVal = (int)UtilGetcJSONNumber(
      cJSON_GetObjectItemCaseSensitive(config, "MsgFlag"));
  autoSend.pktData.canHead.nChnl = (BYTE)nChnIdx;
  cJSON *msgData = cJSON_GetObjectItemCaseSensitive(config, "MsgData");
  autoSend.pktData.canHead.nDataLen =
      UtilJsonByteArray(msgData, &autoSend.pktData.canData[0], DATA_LEN_CANFD);
}

UINT Device::GetGPSRecvNum() { return m_frmGPS.size(); }
...
void Device::GenClearLINAsSlaveResponseRequest(UINT chnIdx,
                                               PacketDataRequest &devRequest) {
...
void Device::GenGetLINTxFIFOInfoRequest(UINT chnIdx,
                                        PacketDataRequest &devRequest) {
...
bool Device::GetFIFOInfoFromJsonStr(std::string &strFIFOInfo,
                                    LINFIFOInfo &fifoInfo) {
  LOG_INFO("Device LIN FIFO info string:%s\n", strFIFOInfo.c_str());

  cJSON *pInfo = cJSON_Parse(strFIFOInfo.c_str());
  if (pInfo) {
    fifoInfo.nChnl = (UINT)UtilGetcJSONNumber(
        cJSON_GetObjectItemCaseSensitive(pInfo, "Chn"));
    fifoInfo.nFIFOSize = (UINT)UtilGetcJSONNumber(
        cJSON_GetObjectItemCaseSensitive(pInfo, "Size"));
    fifoInfo.nFIFOAvailable = (UINT)UtilGetcJSONNumber(
        cJSON_GetObjectItemCaseSensitive(pInfo, "Remain"));
...
void Device::GenSetLINConfigRequest(UINT chnIdx, const LINConfig &linConfig,
                                    PacketDataRequest &devRequest) {
  ...
  devRequest.vecData.assign(strConfig.c_str(),
                            strConfig.c_str() + strConfig.length());
  cJSON_free(pStr);
...
void Device::DealReceivedData(const char *src_addr, const char *data,
                              unsigned int len) {
...
void Device::ProcessPackets(VecPackets &vecPackets) {
  ...
  for (auto &packet : vecPackets) {
    ...
    if (PACKET_TYPE_CAN == pktType) {
      ...
      FrameData vecFrame;
      BYTE pktTypeParam = 0;
      if (CPacketUtil::GetPacketDataDevReq(packet, vecFrame, pktTypeParam) >
          0) {
      ...
      FrameData vecFrame;
      BYTE pktTypeParam = 0;
      if (CPacketUtil::GetPacketDataDevRes(packet, vecFrame, pktTypeParam) >
          0) {
      ...
      if (CPacketUtil::GetPacketDataDevState(packet, vecFrame, pktTypeParam) >
          0) {
      ...
      if (m_bitChnlStarted.test(pkt.canHead.nChnl) || bStartfor1Second) {
        ...
        if (m_bRecvMerge) {
          ...
          if (CPacketUtil::CANFrame2ErrFrame(pkt, errInfo)) {
            CanDataQueue<ZCAN_CHANNEL_ERR_INFO> &errData =
                m_errQueue[pkt.canHead.nChnl];
            errData.lock();
            errData.push(errInfo);
            errData.unlock();
            m_chnlStatus[pkt.canHead.nChnl].regRECounter =
                errInfo.passive_ErrData[1];
            m_chnlStatus[pkt.canHead.nChnl].regTECounter =
                errInfo.passive_ErrData[2];
        ...
        // pkt.canHead.nChnl, m_bitChnlStarted.test(pkt.canHead.nChnl),
        // bStartfor1Second);
      }
    } else {
        ...
        if (m_bRecvMerge) {
          ...
          if (CPacketUtil::CANFrame2ErrFrame(pkt, errInfo)) {
            ...
            if (pkt.canHead.frameInfo.unionVal.bFD) {
              CanDataQueue<PacketDataCANFD> &canData =
                  m_frmCANFD[pkt.canHead.nChnl];
              canData.lock();
              if (canData.push(pkt)) {
              ...
              CanDataQueue<PacketDataCAN> &canData =
                  m_frmCAN[pkt.canHead.nChnl];
              canData.lock();
...
void Device::HandleAuthDataResponse(const FrameData &vecAuth,
                                    BYTE pktTypeParam) {
  std::unique_lock<std::mutex> lk_(m_authMapMutex);
...
void Device::HandleAuthDataRequest(const FrameData &vecAuth,
                                   BYTE pktTypeParam) {
...
void Device::HandleBusUsageFrame(const PacketDataBusUsage &pktBusUsage) {
  if (pktBusUsage.nChnl < m_devChnlCount) {
    ...
    // pktBusUsage.nChnl, pktBusUsage.nBusUsage, pktBusUsage.nFrameCount,
    // pktBusUsage.nTimeStampBegin, pktBusUsage.nTimeStampEnd);
  }
  if (m_bRecvMerge) {
    //新增通道利用率缓存
    CanDataQueue<ZCANDataObj> &canData = m_frmMerge;
...
void Device::HandleDevReqFrame(const FrameData &vecDevReq, BYTE pktTypeParam) {
  ...
  if (reqParam.unionValue.nReqVal == PACKET_TYPE_PARAM_DEV_REQ_RES_TIMESTAMP) {
    ...
    Packet packet =
        CPacketEncoder::BuildPacketDevReqRes(false, nullptr, 0, pktTypeParam);
...
void Device::HandleEchoRequestFrame(const PacketDataCAN &canFrame) {
  LOG_INFO("CAN received, .chnl=%d .tx=%d .echo=%d\n", canFrame.canHead.nChnl,
           canFrame.canHead.frameInfo.unionVal.nTx,
           canFrame.canHead.frameInfo.unionVal.nEcho);
...
void Device::HandleRequestDataEx(BYTE nSeq, PacketDataRequest &requestData) {
  //库接收到其他客户端的请求，模拟设备发出响应
  Packet packet =
      CPacketEncoder::BuildPacketEmulateResponseEx(nSeq, requestData);
  SendData(packet.data(), packet.size());
}
...
// PktType 0x04 设备状态数据
void Device::HandleDevStateData(const FrameData &vecDevState,
                                BYTE pktTypeParam) {
  switch (pktTypeParam) {
...
bool Device::UDSRequest(UINT reqID, const std::string &reqParam,
                        const BYTE *reqData, UINT reqDataLen,
    ...
    while (maxTryCount--) {
      ...
      if (data->responsed) {
        ...
        if (i < data->responseData.vecData.size() - 1) {
          respData.assign(data->responseData.vecData.begin() + i + 1,
                          data->responseData.vecData.end());
        }

        bRet = true;
        LOG_INFO("response data, seq:%d tid:%u/%u\n", nSeq, nTID,
                 data->responseData.nTID);
      } else {
...
bool Device::GetUdsStatus(UINT reqId, UDS_STATUS &status) {
  ...
  std::string strConfig(pStr);
  request.vecData.assign(strConfig.c_str(),
                         strConfig.c_str() + strConfig.length());
  request.vecData.push_back(0);
  cJSON_free(pStr);
  cJSON_Delete(pObj);
  ...
  status = (UDS_STATUS)(int)UtilGetcJSONNumber(
      cJSON_DetachItemFromObjectCaseSensitive(pInfo, "Status"));
  cJSON_Delete(pInfo);
  ...
  if (chnIdx < m_devChnlCount) {
    ...
    for (auto &cfg : m_dynamicCfgData) {
      ...
      for (size_t i = 0; i < tokens.size(); i++) {
        if (tokens.size() == 1) {
          if (tokens[i] == "DevName") {
            cJSON_AddStringToObject(root, tokens[i].c_str(),
                                    cfg.second.c_str());
            continue;
          }
          cJSON_AddNumberToObject(root, tokens[i].c_str(),
                                  std::atoi(cfg.second.c_str()));
          continue;
        }
        ...
        if (iter != m_list.end()) {
          ...
          {
            if (!CheckDynamicCfgIsBaud(tokens[i], cfg.second,
                                       m_list[tokens[i - 1]])) {
              cJSON_AddNumberToObject(m_list[tokens[i - 1]], tokens[i].c_str(),
                                      std::atoi(cfg.second.c_str()));
            }
            continue;
          } else {
            ...
            if (i == 0) {
              ...
              cJSON_AddItemToObject(m_list[tokens[i - 1]], tokens[i].c_str(),
                                    sessionParamNode);
            }
          }
        }
...
cJSON *Device::_GenCANFDFilterCfgJson(const CANFD_FILTER_CFG &rConfig) {
  ...
  for (int i = 0; i < CANFD_FILTER_COUNT_MAX; ++i) {
    ...
    cJSON_AddItemToObject(pRules, key.c_str(),
                          _GenCANFDFilterRuleItemJson(rConfig.vecFilters[i]));
  }
...
cJSON *Device::_GenCANFDFilterRuleItemJson(const CANFD_FILTER_RULE &rConfig) {
  ...
  //或者只接收错误帧时( rConfig.presentFlag.unionValue.bErr == 1 && rConfig.nErr
  //!= 0) 只需要通道参数即可，不再生成后续的配置
  if (!rConfig.presentFlag.unionValue.bErr ||
  ...
  if (rConfig.presentFlag.unionValue.bFilterMask) {
    cJSON_AddItemToObject(
        config, "FilterData",
        _GenJsonByteArray(&rConfig.nFilterData[0],
                          min(rConfig.nFilterDataLen, CANFD_DATA_LEN_MAX)));
    cJSON_AddItemToObject(
        config, "MaskData",
        _GenJsonByteArray(&rConfig.nMaskData[0],
                          min(rConfig.nMaskDataLen, CANFD_DATA_LEN_MAX)));
  }
...
bool Device::SendDynamicCfg(UINT chnIdx, std::string data, UINT isPersist) {
  if (chnIdx < m_devChnlCount) {
    PacketDataRequest request;
    PacketDataResponse response;
    request.nRequest = (isPersist == 0) ? PACKET_REQUEST_SET_CONFIG
                                        : PACKET_REQUEST_SET_CONFIG_PERSIST;
    request.nTID = 0;
...
bool Device::CheckDynamicCfgIsBaud(std::string cfg, std::string value,
                                   cJSON *obj) {
  if (!m_bDevInfoUpdated) {
  ...
  if (cfg == "NominalBaud" || cfg == "DataBaud") {
    ...
    if (str_hw_type == "CANFDDTU-300UR-MINI" ||
        str_hw_type == "CANFDDTU-200UR" || str_hw_type == "CANFDDTU-300EWGR" ||
        str_hw_type == "CANFDDTU-300ER") {
      if (cfg == "NominalBaud") {
        int size = sizeof(kCANFDDTU300AbitBaudrate) / sizeof(BaudTiming);
        for (int i = 0; i < size; i++) {
          if (kCANFDDTU300AbitBaudrate[i].nBaudVal ==
              std::atoi(value.c_str())) {
            cJSON_AddNumberToObject(sessionParamNode, "TSEG1",
                                    kCANFDDTU300AbitBaudrate[i].nTSEG1);
            cJSON_AddNumberToObject(sessionParamNode, "TSEG2",
                                    kCANFDDTU300AbitBaudrate[i].nTSEG2);
            cJSON_AddNumberToObject(sessionParamNode, "SJW",
                                    kCANFDDTU300AbitBaudrate[i].nSJW);
            cJSON_AddNumberToObject(sessionParamNode, "BRP",
                                    kCANFDDTU300AbitBaudrate[i].nBRP);
          }
        }
      }
      if (cfg == "DataBaud") {
        ...
        for (int i = 0; i < size; i++) {
          if (kCANFDDTU300DbitBaudrate[i].nBaudVal ==
              std::atoi(value.c_str())) {
            cJSON_AddNumberToObject(sessionParamNode, "TSEG1",
                                    kCANFDDTU300DbitBaudrate[i].nTSEG1);
            cJSON_AddNumberToObject(sessionParamNode, "TSEG2",
                                    kCANFDDTU300DbitBaudrate[i].nTSEG2);
            cJSON_AddNumberToObject(sessionParamNode, "SJW",
                                    kCANFDDTU300DbitBaudrate[i].nSJW);
            cJSON_AddNumberToObject(sessionParamNode, "BRP",
                                    kCANFDDTU300DbitBaudrate[i].nBRP);
          }
        }
      }
    } else {
      if (cfg == "NominalBaud") {
        ...
        for (int i = 0; i < size; i++) {
          if (kUSBCANFD800AbitBaudrate[i].nBaudVal ==
              std::atoi(value.c_str())) {
            cJSON_AddNumberToObject(sessionParamNode, "TSEG1",
                                    kUSBCANFD800AbitBaudrate[i].nTSEG1);
            cJSON_AddNumberToObject(sessionParamNode, "TSEG2",
                                    kUSBCANFD800AbitBaudrate[i].nTSEG2);
            cJSON_AddNumberToObject(sessionParamNode, "SJW",
                                    kUSBCANFD800AbitBaudrate[i].nSJW);
            cJSON_AddNumberToObject(sessionParamNode, "BRP",
                                    kUSBCANFD800AbitBaudrate[i].nBRP);
          }
        }
      }
      if (cfg == "DataBaud") {
        ...
        for (int i = 0; i < size; i++) {
          if (kUSBCANFD800DbitBaudrate[i].nBaudVal ==
              std::atoi(value.c_str())) {
            cJSON_AddNumberToObject(sessionParamNode, "TSEG1",
                                    kUSBCANFD800DbitBaudrate[i].nTSEG1);
            cJSON_AddNumberToObject(sessionParamNode, "TSEG2",
                                    kUSBCANFD800DbitBaudrate[i].nTSEG2);
            cJSON_AddNumberToObject(sessionParamNode, "SJW",
                                    kUSBCANFD800DbitBaudrate[i].nSJW);
            cJSON_AddNumberToObject(sessionParamNode, "BRP",
                                    kUSBCANFD800DbitBaudrate[i].nBRP);
          }
        }
      }
    }
...
bool Device::ApplyCANFilter() {
  ...
  for (int i = 0; i < DEV_CHNL_COUNT_MAX; i++) {
    if (!m_vecFilterItems[i].empty()) {
      filter_item.insert(filter_item.end(), m_vecFilterItems[i].begin(),
                         m_vecFilterItems[i].end());
    }
  }
...
bool Device::SyncDevClock() {
  ...
  SYSTEMTIME stTime;
  GetLocalTime(&stTime);
  nLen = sprintf_s(bufTime, "%04d-%02d-%02dT%02d:%02d:%02d.%03d", stTime.wYear,
...
ZCAN_RET_STATUS Device::UDS_Request(const ZCAN_UDS_REQUEST *req,
                                    ZCAN_UDS_RESPONSE *resp, BYTE *dataBuf,
                                    UINT dataBufSize) {
  ...
  cJSON_AddNumberToObject(sessionParamNode, "Timeout",
                          req->session_param.timeout);
  cJSON_AddNumberToObject(sessionParamNode, "EnhancedTimeout",
                          req->session_param.enhanced_timeout);
  cJSON_AddNumberToObject(sessionParamNode, "ChkAnyNegResp",
                          req->session_param.check_any_negative_response);
  cJSON_AddNumberToObject(sessionParamNode, "WaitIfSuppressResp",
                          req->session_param.wait_if_suppress_response);
  ...
  cJSON_AddNumberToObject(transParamNode, "MaxDataLen",
                          req->trans_param.max_data_len);
  cJSON_AddNumberToObject(transParamNode, "LocalStMin",
                          req->trans_param.local_st_min);
  cJSON_AddNumberToObject(transParamNode, "BlockSize",
                          req->trans_param.block_size);
  cJSON_AddNumberToObject(transParamNode, "FillByte",
                          req->trans_param.fill_byte);
  cJSON_AddNumberToObject(transParamNode, "Ext", req->trans_param.ext_frame);
  cJSON_AddNumberToObject(transParamNode, "IsModifyEcuStMin",
                          req->trans_param.is_modify_ecu_st_min);
  cJSON_AddNumberToObject(transParamNode, "RemoteStMin",
                          req->trans_param.remote_st_min);
  cJSON_AddNumberToObject(transParamNode, "FCTimeout",
                          req->trans_param.fc_timeout);
  cJSON_AddNumberToObject(transParamNode, "FillMode",
                          req->trans_param.fill_mode);
  ...
  LOG_INFO("uds request: udsId:%u, len:%u, data:%02x %02x ...", req->req_id,
           req->data_len + 1, req->sid, req->data_len > 0 ? req->data[0] : 0);

  if (!UDSRequest(req->req_id, reqParam, req->data, req->data_len, respParam,
                  respData)) {
  ...
  resp->status = (UINT)UtilGetcJSONNumber(
      cJSON_GetObjectItemCaseSensitive(respRoot, "Status"));
  resp->type = (UINT)UtilGetcJSONNumber(
      cJSON_GetObjectItemCaseSensitive(respRoot, "RespType"));

  if (resp->status == ZCAN_UDS_ERROR_OK) {
    if (resp->type == ZCAN_UDS_RT_POSITIVE) {
      resp->positive.sid = (UINT)UtilGetcJSONNumber(
          cJSON_GetObjectItemCaseSensitive(respRoot, "SID"));
      resp->positive.data_len = (UINT)UtilGetcJSONNumber(
          cJSON_GetObjectItemCaseSensitive(respRoot, "DateLen"));
      if (respData.size() > 0) {
        if (dataBuf) {
      ...
      resp->negative.neg_code = (UINT)UtilGetcJSONNumber(
          cJSON_GetObjectItemCaseSensitive(respRoot, "NegCode"));
      resp->negative.sid = (UINT)UtilGetcJSONNumber(
          cJSON_GetObjectItemCaseSensitive(respRoot, "SID"));
      resp->negative.error_code = (UINT)UtilGetcJSONNumber(
          cJSON_GetObjectItemCaseSensitive(respRoot, "NegErr"));
    }
  }
...
ZCAN_RET_STATUS Device::UDS_Control(const ZCAN_UDS_CTRL_REQ *ctrl,
  ...
  memset(resp, 0, sizeof(resp));
  resp->result = (UINT)UtilGetcJSONNumber(
      cJSON_GetObjectItemCaseSensitive(respRoot, "Status"));
  cJSON_Delete(respRoot);
...
ZCAN_RET_STATUS Device::UDS_RequestEX(const ZCANUdsRequestDataObj *requestData,
                                      ZCAN_UDS_RESPONSE *resp, BYTE *dataBuf,
                                      UINT dataBufSize) {
  ...
  do {
    ...
    case DEF_CAN_UDS_DATA:
      ret = UDS_Request(requestData->data.zcanCANFDUdsData.req, resp, dataBuf,
                        dataBufSize);
      break;
    case DEF_LIN_UDS_DATA:
      // ret = UDS_RequestLIN(requestData->data.zcanLINUdsData.req, resp,
      // dataBuf, dataBufSize);
      break;
    default:
      break;
...
ZCAN_RET_STATUS Device::UDS_ControlEX(ZCAN_UDS_DATA_DEF dataType,
