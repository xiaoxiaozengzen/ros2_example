...
template <class _Ty> class CanDataQueue : public std::queue<_Ty> {
public:
  ...
  _Ty front() { return std::queue<_Ty>::front(); }
...
/*
    ...
    队列总内存的使用量。
*/
template <class T> class CanDataQueue {
private:
  size_t m_capacity_max; //最大容量
  size_t
      m_capacity; //当前容量，在达到最大容量之前，随着队列数据增多，会慢慢增加到最大容量
...
public:
  CanDataQueue(size_t maxCount = CAN_DATA_QUEUE_BUFF_SIZE)
      : m_capacity(CAN_DATA_QUEUE_BUFF_INIT), m_capacity_max(maxCount),
        m_size(0), m_front(0), m_rear(0) {
  ...
  bool isFull() { return m_size > 0 && m_size == m_capacity; }

  bool isMaxCapacity() { return m_capacity == m_capacity_max; }
  ...
  void reallocMemory() {
    if (!isMaxCapacity()) {
      ...
      if (size_1 != m_size) {
#ifdef WIN32
        LOG_ALWAYS("tid = %u, realloc to %d, cap:%d cap_max:%d front:%d "
                   "rear:%d size:%d ",
                   GetCurrentThreadId(), size_1, m_capacity, m_capacity_max,
                   m_front, m_rear, m_size);
#endif
        T *p = new T[size_1];
        if (p != nullptr) {
...
#ifdef WIN32
        LOG_ALWAYS("tid = %u，realloc done, cap:%d cap_max:%d front:%d rear:%d "
                   "size:%d ",
                   GetCurrentThreadId(), m_capacity, m_capacity_max, m_front,
                   m_rear, m_size);
#endif
      }
    }
  ...
  const T &front() { return m_data[m_front]; }
...
struct DevChnlAutoSendList {
  struct {
    std::chrono::steady_clock::time_point
        m_tpRecv; //获取设备数据时间，如果短时间内多次请求，则直接使用缓存的数据
...
public:
  ...
  ULONG Receive(UINT chnIdx, ZCAN_Receive_Data *pReceive, ULONG len,
                int waitTime = -1);
  ULONG ReceiveFD(UINT chnIdx, ZCAN_ReceiveFD_Data *pReceive, ULONG len,
                  int waitTime = -1);
  ...
  UINT SetLINSubscribe(UINT chnIdx, PZCAN_LIN_SUBSCIBE_CFG pSend,
                       UINT nMsgCount);
  UINT SetLINPublish(UINT chnIdx, PZCAN_LIN_PUBLISH_CFG pSend, UINT nMsgCount);
  // UDS
  ZCAN_RET_STATUS UDS_Request(const ZCAN_UDS_REQUEST *req,
                              ZCAN_UDS_RESPONSE *resp, BYTE *dataBuf,
                              UINT dataBufSize);
  ZCAN_RET_STATUS UDS_Control(const ZCAN_UDS_CTRL_REQ *ctrl,
                              ZCAN_UDS_CTRL_RESP *resp);
  ZCAN_RET_STATUS UDS_RequestEX(const ZCANUdsRequestDataObj *requestData,
                                ZCAN_UDS_RESPONSE *resp, BYTE *dataBuf,
                                UINT dataBufSize);
  ZCAN_RET_STATUS UDS_ControlEX(ZCAN_UDS_DATA_DEF dataType,
...
private:
  void DealReceivedData(const char *src_addr, const char *data,
                        unsigned int len);
  ...
  AutoSendData *GetAutoSendIndex(VecAutoSendData &vecAutoSend,
                                 UINT autoSendIndex);
  void ZCANAutoSend2AutoSend(const ZCAN_AUTO_TRANSMIT_OBJ *pObj,
                             AutoSendData &autoSend);
  void ZCANAutoSend2AutoSend(const ZCANFD_AUTO_TRANSMIT_OBJ *pObj,
                             AutoSendData &autoSend);
  ...
  bool RequestDevData(PacketDataRequest &devRequest,
  ...
  void GenGetAutoSendListRequest(UINT chnIdx, UINT nOffset, UINT nRequestCount,
  ...
  bool GetDevState(
      BYTE nType,
      std::string &strDevState); //根据类型主动获取设备状态信息 nType取值为
                                 // PACKET_TYPE_DEV_STATE_PARAM_SYS_INFO 等数值
  bool GetDevAutoSendList(UINT chnIdx,
  ...
  bool GetDevInfoFromJsonStr(const std::string &strDevInfo,
                             ZCAN_DEVICE_INFO *pInfo);
  UINT GetAutoSendListFromJsonStr(UINT chnIdx, UINT nOffset, UINT nRequestCount,
                                  const std::string &strAutoSendJson,
                                  std::vector<PacketDataAutoSend> &vecAutoSend);
  void GetAutoSendFromJson(UINT nChnIdx, UINT nIndex, cJSON *config,
  ...
  void GenClearLINAsSlaveResponseRequest(UINT chnIdx,
                                         PacketDataRequest &devRequest);
  void GenGetLINTxFIFOInfoRequest(UINT chnIdx, PacketDataRequest &devRequest);
  void GenSetLINConfigRequest(UINT chnIdx, const LINConfig &linConfig,
                              PacketDataRequest &devRequest);
  ...
  // UDS
  bool UDSRequest(UINT reqID, const std::string &reqParam, const BYTE *reqData,
                  UINT reqDataLen, std::string &respParam,
  ...
  bool setDynamicCfg(UINT chnIdx,
                     const ZCAN_DYNAMIC_CONFIG *pObj); //暂时只给设置滤波使用
...
private:
  ...
  std::bitset<DEV_CHNL_COUNT_MAX + DEV_LIN_COUNT_MAX>
      m_bitChnlStarted; //设备通道是否启动，包含CAN+LIN,CAN通道索引从0开始，LIN通道索引从
                        // DEV_CHNL_COUNT_MAX开始
  std::chrono::steady_clock::time_point
      m_tpDevStartTickCount; //设备建立连接时的时间，避免设备刚建立连接，通道尚未全部打开时设备立即发送数据可能造成的数据丢失。
  ...
  ZCAN_DEVICE_INFO
  m_stDevInfo; // 设备信息字符，tcp客户端连接启动后会从设备获取，udp，tcp
               // server模式不做获取
  bool m_bDevInfoUpdated;    // 用户指示设备信息 m_stDevInfo
                             // 是否是从设备交互获取到的
