...
const uint8_t PACKET_TYPE_DEV_STATE =
    0x04; // 设备状态包，客户端配置开启后，设备周期上传数据，只能由设备上报，数据区为JSON字符串
const uint8_t PACKET_TYPE_GPS =
    0x05; // GPS数据包，只能由设备上报，包类型参数保留，长度为n*GPS结构
const uint8_t PACKET_TYPE_LIN =
    0x06; // LIN数据包，设备可以上报，包类型参数保留，长度为n*LIN结构
const uint8_t PACKET_TYPE_REQUEST_RESPONSE = 0x50; // 客户端发起请求，设备端回应
const uint8_t PACKET_TYPE_REQUEST_RESPONSE_EX =
    0x52; // 客户端发起请求，设备端回应（扩展格式）
...
const uint8_t PACKET_TYPE_PARAM_FLAG_COMPRESS =
    0x01; // 包类型为PACKET_TYPE_CAN，PACKET_TYPE_CANFD时，包类型参数Bit0表示数据是否压缩(zlib压缩)，仅压缩数据区域
...
typedef std::vector<uint8_t>
    Packet; //用于表示一个包的数据，包含 PacketHead + Data + 1BYTE Checksum
typedef std::vector<Packet> VecPackets; //包含N个完整的Packet
typedef std::vector<uint8_t>
    FrameData; //用于表示有效数据，PacketDataCAN/PacketDataCANFD/AUTH DATA
...
typedef union _tagCANFrameInfo {
  struct {
    USHORT nSendtype : 2; // bit 0-1, 0:正常发送; 1:单次发送(CANFDNET不支持);
                          // 2:自发自收 (发送有效，接收为0)
    USHORT
    nTx : 1; // bit 2, 2019.11.26,
             // 接收有效，0：表示设备正常接收数据帧；1：表示设备发送回显(Echo)帧
    USHORT
    nEcho : 1; // bit 3, 2019.11.26,
               // 发送有效，0：表示设备正常发送不需要回显；1：表示设备发送数据需要回显(Echo)帧
    ...
    USHORT
    bSendDelay : 1; // bit10, 2020.7.17，0:普通发送;
                    // 1:发送后延时，发送当前帧时开始计时，到达时间后从队列中的取出下一帧发送
...
typedef struct _LINTxInfo {
  union {
    struct {
      USHORT bSendMode : 2; // Bit0~1: 发送方式： 0–订阅数据（此时根据报文长度
                            // Length接收数据）1-从机应答数据
                            // 2-发送主机请求、仅发送报头
                            // 3、主机发送请求及数据（报头+从机数据）
...
typedef struct _LINRxInfo {
  union {
    struct {
      ...
      USHORT
      bErrStage : 4; // bit2-5：错误阶段，ERR为1时有效 0-总线空闲
                     // 1-Break段 2-同步段 3-ID段
                     // 4~12-数据错误（包括校验），表示接收哪个字节时出错，4表示字节0
                     // 15-未知
      USHORT
      bErrReason : 4; // bit6-9: 错误原因，ERR为1有效 0-接收超时
                      // 1-接收到不合法（不符合协议）数据，如ID校验错误、同步段错误、校验错误。此时报文中对应字段表示该错误实际值
                      // 2-接收到字节错误（停止位为显性） 3-Break段错误
                      // 15-未知错误
...
typedef union _tagZLGCAN_PASSIVE_ERRORINFO_BYTE {
  struct {
    ...
    uint8_t
        nErrCodeType : 2; //错误代码类型, 0:位错；1:格式错；2:填充错；3:其他错
  } unionValue;
  uint8_t nRawValue;
#ifdef _WIN32
...
typedef union _tagPacketDataAutoSendFlag {
  struct {
    ...
    USHORT
    nUseDataTimestamp : 1; // Bit1,为1表示发送周期使用报文中timestamp字段低4字节，单位
                           // 100us,为0表示使用nPeriodMs字段，单位ms，1-60000ms
...
typedef struct _tagPacketDataGPS {
  UINT64
  nTimestamp; //时间戳，Bit0-55表示UTC时间戳，单位us，Bit56-62保留，Bit63表示数据是否有效，1表示有效，0表示无效定位，沿用上次有效定位数据
  UINT
      nLongitude; //经度,Bit0-27以度为单位值乘以10的6次方，精确到百万分之一度，
                  // bit28-29保留，bit30为0表示东经，为1表示西经，bit31表示数据是否有效，1表示有效，0表示无效定位，沿用上次有效定位数据
  UINT
      nLatitude; //纬度,Bit0-27以度为单位值乘以10的6次方，精确到百万分之一度，
                 // bit28-29保留，bit30为0表示北纬，为1表示南纬，bit31表示数据是否有效，1表示有效
  UINT nSpeed; //速度，Bit0-15，单位0.1Km/h,取值0-10000表示0Km/h-1000.0km/h,
               // bit31表示数据是否有效，1表示有效
  UINT nCourseAngle; //航向角,Bit0-30,以度为单位乘以10的6次方，
                     // bit31表示数据是否有效，1表示有效
  UINT
      nAltitude; //海拔，Bit0-15，单位米，偏移1000m，bit31表示数据是否有效，1表示有效
  UINT nReserved; //保留
#ifdef _WIN32
} PacketDataGPS;
#pragma pack(pop)
...
typedef union _tagGPSSpdAltitude {
  struct {
    UINT nData : 16;     // bit0-15, 速度单位0.1Km/h，范围0-10000(1000.0km/h)；
                         // 高度单位米，偏移1000m
...
// const uint8_t PACKET_TYPE_REQUEST_RESPONSE = 0x50;
// 包类型为0x50时的请求，应答数据
...
const uint8_t PACKET_REQUEST_GET_DEVICE_DELAY_SEND_STATE =
    4;                                                   // 获取延时队列状态
const uint8_t PACKET_REQUEST_CLEAR_DELAY_SEND_QUEUE = 5; // 清空发送延时队列
const uint8_t PACKET_REQUEST_SET_CONFIG = 6;             // 设置动态配置
const uint8_t PACKET_REQUEST_CLEAR_LIN_AS_SLAVE_RESPONSE =
    7; // 清空LIN作为从站时的应答数据
...
struct LINConfig {
  ...
  union {
    struct {
      UINT bEnhancedChksum : 1; // Bit0:
                                // 增强型校验，1-增强型校验和，0-经典型校验和
      UINT bVarDLCDisabled : 1; // Bit1:  关闭可变消息长度，1-关闭可变
                                // DLC，0-可变 DLC
                                // ，关闭可变消息长度后，消息长度将不再依赖消息
                                // ID
      UINT nReserved : 30;      // Bit2~31:  保留，为 0
    } unionVal;
    UINT rawVal; // 注：对于 LIN2.X，Bit0和 Bit1 均设为 1,对于LIN1.X，Bit0和
                 // Bit1 均设为 0
  } stFeature;   // LIN 特性位
};
...
const int PACKET_LEN_MIN =
    PACKET_HEAD_LEN + 1; //帧头(6Byte)+数据(0)+校验(1Byte)
const int PACKET_REQUEST_RESPONSE_DATALEN_MIN =
    2; // const uint8_t PACKET_TYPE_REQUEST_RESPONSE = 0x50
       // 时，Data部分最短长度为2，包含 1Byte tid 和 1Byte result
const int PACKET_REQUEST_RESPONSE_EX_DATALEN_MIN =
    5; // const uint8_t PACKET_TYPE_REQUEST_RESPONSE_EX = 0x52
       // 时，Data部分最短长度为5，包含 4Byte tid 和 1Byte result
const int PACKET_DATA_LEN_DECOMPRESS_MAX =
    10000 *
    FRAME_LEN_CANFD; //一个包包含压缩数据解压后最大长度，暂定为10000帧CANFD
...
public:
  ...
  uint8_t front() { return data_[beg_index_]; }

  void clear() { size_ = beg_index_ = end_index_ = 0; }
...
// typedef std::deque<uint8_t>    PacketBuffer;
// //双端队列，用于缓存接收到的数据并进行处理，数据从队列尾部进入，处理完毕的从队列头部删掉
...
public:
  ...
  static int GetPacketDataCAN(const Packet &packet,
                              std::vector<PacketDataCAN> &vecFrame);
  static int GetPacketDataCANFD(const Packet &packet,
                                std::vector<PacketDataCANFD> &vecFrame);
  static int GetPacketDataGPS(const Packet &packet,
                              std::vector<PacketDataGPS> &vecFrame);
  static int GetPacketDataLIN(const Packet &packet,
                              std::vector<PacketDataLIN> &vecFrame);
  static int GetPacketDataAUTH(const Packet &packet, FrameData &vecAuth,
                               uint8_t &pktTypeParam);
  static int GetPacketDataBusUsage(const Packet &packet,
                                   PacketDataBusUsage &pktBusUsage);
  static int GetPacketDataDevReq(const Packet &packet, FrameData &vecBusDevReq,
                                 uint8_t &pktTypeParam);
  static int GetPacketDataDevRes(const Packet &packet, FrameData &vecBusDevRes,
                                 uint8_t &pktTypeParam);
  static int GetPacketDataRequest(const Packet &packet,
                                  PacketDataRequest &request);
  static int GetPacketDataRequestEx(const Packet &packet,
                                    PacketDataRequest &request);
  static int GetPacketDataResponse(const Packet &packet,
                                   PacketDataResponse &response);
  static int GetPacketDataResponseEx(const Packet &packet,
                                     PacketDataResponse &response);
  static int GetPacketDataDevState(const Packet &packet, FrameData &vecDevStat,
                                   uint8_t &pktTypeParam);
  ...
  static bool GetPacket(const FrameData &pktFrame,
                        PacketDataAutoSend &pktAutoSend);

  static UINT64 GetNowTimeStampAsMicrosecond();
  static void ZCANTx2CANFrame(const ZCAN_Transmit_Data &zTx, int nChnl,
                              bool bTxEchoEnable, PacketDataCAN &pktFrame);
  static void ZCANTx2CANFrame(const ZCAN_TransmitFD_Data &zTx, int nChnl,
                              bool bTxEchoEnable, PacketDataCANFD &pktFrame);
  static bool ZCANDataObj2CANFrame(const ZCANDataObj &zTx,
                                   PacketDataCANFD &pktFrame);

  static void CANFrame2ZCANRx(const PacketDataCAN &pktFrame,
                              ZCAN_Receive_Data &zRx, int *pChnl);
  static void CANFrame2ZCANRx(const PacketDataCAN &pktFrame, ZCANDataObj &zRx);
  static void CANFrame2ZCANRx(const PacketDataCANFD &pktFrame,
                              ZCAN_ReceiveFD_Data &zRx, int *pChnl,
                              bool bRecvMerge);
  static void CANFrame2ZCANRx(const PacketDataCANFD &pktFrame,
                              ZCANDataObj &zRx);

  static void CANFrame2CANFDFrame(const PacketDataCAN &pktFrameCAN,
                                  PacketDataCANFD &pktFrameCANFD);
  static void CANFDFrame2CANFrame(const PacketDataCANFD &pktFrameCANFD,
                                  PacketDataCAN &pktFrameCAN);

  static bool CANFrame2ErrFrame(const PacketDataCAN &pktFrame,
                                ZCAN_CHANNEL_ERR_INFO &errInfo);
  static bool CANFrame2ErrFrame(const PacketDataCANFD &pktFrame,
                                ZCAN_CHANNEL_ERR_INFO &errInfo);

  static void ZCANAutoSend2AutoSendFrame(const ZCAN_AUTO_TRANSMIT_OBJ &zCAN,
                                         int nChnl, bool bTxEchoEnable,
                                         PacketDataAutoSend &autoSend);
  static void ZCANAutoSend2AutoSendFrame(const ZCANFD_AUTO_TRANSMIT_OBJ &zCAN,
                                         int nChnl, bool bTxEchoEnable,
                                         PacketDataAutoSend &autoSend);
  static void AutoSendData2AutoSendFrame(const AutoSendData &autoSendData,
                                         int nChnl, bool bTxEchoEnable,
                                         PacketDataAutoSend &autoSendFrame);
  static void AutoSendFrame2ZCANAutoSend(const PacketDataAutoSend &autoSend,
  ...
  static void PacketGPS2ZCANDataObj(const PacketDataGPS &pktGPS,
                                    ZCANDataObj &zGPS);

  // LIN
  static void PacketLIN2ZLIN(const PacketDataLIN &pktLIN, ZCAN_LIN_MSG &zLIN,
                             int *pChnl);
  static void PacketLIN2ZCANDataObj(const PacketDataLIN &rProtocalFrame,
                                    ZCANDataObj &rZCANDataObj);
  static void ZLIN2PacketLIN(int nChnl, const ZCAN_LIN_MSG &zLIN,
                             PacketDataLIN &pktLIN);
  static bool ZCANDataObj2PacketLIN(const ZCANDataObj &zTx,
                                    PacketDataLIN &pktLIN);

  static void ZLIN2PacketLIN(int nChnl, const ZCANLINData &zcanLINData,
                             PacketDataLIN &pktLIN);
  static void ZLIN2PacketLIN(int nChnl,
                             const ZCAN_LIN_SUBSCIBE_CFG &zLINSubscibe,
                             PacketDataLIN &pktLIN);
  static void ZLIN2PacketLIN(int nChnl,
                             const ZCAN_LIN_PUBLISH_CFG &zLINResponse,
                             PacketDataLIN &pktLIN);
  // static void ZLIN2PacketLIN(int nChnl, const ZCAN_LIN_SUBSCIBE_CFG
  // &zLINSubscibe, PacketDataLIN& pktLIN);

  static bool CANErrorData2ZcanErrInfo(const PacketDataCANHead &canHead,
                                       const uint8_t *data,
                                       ZCAN_CHANNEL_ERR_INFO &errInfo);
  static bool CANErrorData2ZCANDataObj(const PacketDataCANHead &canHead,
                                       const uint8_t *data,
                                       ZCANDataObj &errInfo);
  ...
  //通道利用率
  static void
  PacketDataBusUsage2ZCANDataObj(const PacketDataBusUsage &pktBusUsage,
                                 ZCANDataObj &zRx);

  static VecVecZCANDataObjPtr SplitZCANData(const ZCANDataObj *pSend,
                                            ULONG len);
  static bool PacketData2ZCANDataObj(const PacketDataCANFD &pktFrame,
                                     ZCANDataObj &zCANObj);
  static bool PacketData2ZCANDataObj(const PacketDataCAN &pktFrame,
                                     ZCANDataObj &zCANObj);
  static bool PacketData2ZCANDataObj(const PacketDataGPS &pktFrame,
                                     ZCANDataObj &zCANObj);
  static bool PacketData2ZCANDataObj(const PacketDataLIN &pktFrame,
                                     ZCANDataObj &zCANObj);
  static bool PacketData2ZCANDataObj(const PacketDataBusUsage &pktBusUsage,
                                     ZCANDataObj &zCANObj);
};
...
public:
  static VecPackets BuildPacketCAN(int nChnl, const ZCAN_Transmit_Data *pSend,
                                   ULONG len, bool bTxEchoEnable);
  static VecPackets BuildPacketCANFD(int nChnl,
  ...
  static Packet BuildPacketAUTH(bool bRequest, const uint8_t *data,
                                unsigned int len, uint8_t autoIncIndex);
  static Packet BuildPacketAutosend(int nChnl,
                                    const ZCAN_AUTO_TRANSMIT_OBJ *pObj,
                                    bool bTxEchoEnable);
  static Packet BuildPacketAutosend(int nChnl,
                                    const ZCANFD_AUTO_TRANSMIT_OBJ *pObj,
                                    bool bTxEchoEnable);
  static VecPackets BuildPacketAutosend(int nChnl,
                                        const VecAutoSendData &vecAutoSend,
                                        bool bTxEchoEnable);
  static Packet BuildPacketDevReqRes(bool bRequest, const uint8_t *data,
                                     unsigned int len, uint8_t pktTypeParam);
  static Packet BuildPacketRequest(uint8_t nSeq,
                                   const PacketDataRequest &request);
  static Packet BuildPacketRequestEx(uint8_t nSeq,
                                     const PacketDataRequest &request);
  static Packet
  BuildPacketEmulateResponse(uint8_t nSeq,
                             const PacketDataRequest &requestData);
  static Packet
  BuildPacketEmulateResponseEx(uint8_t nSeq,
                               const PacketDataRequest &requestData);
  static void
  GenRequestResponseEmulateData(const PacketDataRequest &requestData,
                                PacketDataResponse &responseData);
  static VecPackets BuildPacketLIN(int nChnl, const PZCAN_LIN_MSG pSend,
                                   ULONG len);
  static VecPackets BuildPacketLIN(const VecZCANDataObjPtr &vecData);
  // 2022.11.12新加
  static VecPackets
  BuildPacketLIN(int nChnl, const PZCAN_LIN_SUBSCIBE_CFG pSend, ULONG len);
  static VecPackets BuildPacketLIN(int nChnl, const PZCAN_LIN_PUBLISH_CFG pSend,
                                   ULONG len);
...
private:
  static PacketHead BuildPacketHead(uint8_t pktType, int pktCnt,
