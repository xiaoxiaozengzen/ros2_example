...
TcpServer::TcpServer(unsigned short svrPort, unsigned int maxClient)
    : m_svrPort(svrPort), m_maxClient(maxClient) {
...
bool TcpServer::IsStart() { return m_bStarted; }
...
void TcpServer::RecvThdLoop() {
  ...
  while (!m_stop) {
    ...
    int ret = cc_socket_select(&m_socket, &m_clients, &setted,
                               0); // -1 block, 0 return immediately
    if (ret == 0) {
          ...
          if (m_clients[j].handle == setted[i].handle) {
            ...
            if (len < 0) {
              ...
              if (m_recvCb) {
                ...
                snprintf(src_addr, sizeof(src_addr) - 1, "%s:%d", curSock->ip,
                         curSock->port);
...
const std::vector<cc_socket> *TcpServer::GetClients() { return &m_clients; }
