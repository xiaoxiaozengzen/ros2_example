...
UdpService::UdpService(unsigned short localPort, const char *rmtIp,
                       unsigned short rmtPort, int multicastloopback)
    : m_localPort(localPort), m_rmtIp(rmtIp), m_rmtPort(rmtPort),
      m_multicastloopback(multicastloopback) {
...
bool UdpService::IsStart() { return m_bStarted; }
...
void UdpService::RecvThdLoop() {
  while (!m_stop) {
    ...
    int len =
        cc_socket_recv_from(&m_socket, m_recvbuf, UDP_RECV_BUFF_SIZE, &clt);
    if (len > 0) {
      LOG_INFO("UDP recv %d bytes from %s:%d\n", len, clt.ip, clt.port);
      if (!m_bmulticast) {
        // UDP单播不接受非目的ip和端口的数据
        if (m_rmtIp.compare(clt.ip) != 0 || m_rmtPort != clt.port) {
...
bool UdpService::IsIpMulticast(const char *rmtIp) {
  ...
  if (longaddr >= 0xe0000000 && longaddr <= 0xefffffff) {
    return true;
  }
  return false;
}
