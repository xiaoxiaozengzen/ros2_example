...
TcpClient::TcpClient(const char *svrIp, unsigned short svrPort, UINT devType)
    : m_svrIp(svrIp), m_svrPort(svrPort), m_devType(devType) {
...
bool TcpClient::IsStart() { return m_bStarted; }
...
bool TcpClient::ConnectServer() {
  ...
  cc_socket_set_recv_snd_buf(&m_socket, socket_recv_buf_size,
                             socket_send_buf_size);

  LOG_INFO("TCP connect %s:%d ...\n", m_svrIp.c_str(), m_svrPort);

  if (cc_socket_connect(&m_socket, m_svrIp.c_str(), m_svrPort) !=
      CC_SOCKET_OK) {
...
bool TcpClient::DisconnectServer() {
  ...
  double time = ddt / ddf;
  // std::cout << __FUNCTION__ << "^^^^ joinable time is: " << time * 1000 <<
  // "ms" << std::endl; LOG_ALWAYS("%s^^^^ DisconnectServer time is: %fms",
  // __FUNCTION__, time * 1000);
#endif
  ...
  while (!m_stopRecvLoop) {
    ...
    if (len > 0) {
      if (m_recvCb) {
        ...
        snprintf(src_addr, sizeof(src_addr) - 1, "%s:%d", m_socket.ip,
                 m_socket.port);
      ...
      while (1) {
        ...
        if (ReconnectServer()) {
          if (m_devType == ZCAN_CANFDNET_TCP ||
              m_devType == ZCAN_CANFDNET_UDP ||
              ...
              m_devType == ZCAN_CANFDNET_800U_UDP) { // 同步设备时间
            char bufTime[50];
            int nLen;
#ifdef WIN32
            SYSTEMTIME stTime;
            GetLocalTime(&stTime);
            nLen = sprintf_s(bufTime, "%04d-%02d-%02dT%02d:%02d:%02d.%03d",
            ...
            SendData(reinterpret_cast<const char *>(packet.data()),
                     packet.size());
          }
          break;
        }
...
bool TcpClient::Stop() { return DisconnectServer(); }
