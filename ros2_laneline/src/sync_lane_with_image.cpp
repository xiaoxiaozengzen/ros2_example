...
public:
    ...
    cv::Mat avframe_to_bgr_mat(AVFrame* frame) {
        ...
        auto print_rgb_fomat = [&, this]() {
            ...
            /**
            * @brief 分配一个w*h大小的，pix_fmt格式的图像缓冲区,并填充data跟linesize
            * @param pointers 大小为4的uint8_t*数组，存储图像数据的每个个平面的起始地址
            * @param linesize 大小为4的整型数组，每个平面的行大小（以字节为单位）
            * @param w 图像宽度
            * @param h 图像高度
            * @param pix_fmt 像素格式
            * @param align 对齐方式，通常为1
            *
            * @note 如果output_frame之前已经分配过内存，需要先调用av_freep(&output_frame->data[0])释放之前的内存，避免内存泄漏
            */
            ...
            if (ret < 0) {
                ...
                throw std::runtime_error("Could not allocate output image.");
            }
            ...
            /**
                ...
                */
            int numBytes = av_image_get_buffer_size((AVPixelFormat)frame->format, frame->width, frame->height, 1);
            std::cout << "YUV420P buffer size: " << numBytes << std::endl;
            numBytes = av_image_get_buffer_size((AVPixelFormat)output_frame->format, output_frame->width, output_frame->height, 1);
            std::cout << "BGR24 buffer size: " << numBytes << std::endl;
            std::cout << "Output Frame format: " << av_get_pix_fmt_name((AVPixelFormat)output_frame->format) << std::endl;
            std::cout << "Output Frame Linesize 0: " << output_frame->linesize[0] << ", Linesize 1: " << output_frame->linesize[1] << ", Linesize 2: " << output_frame->linesize[2] << std::endl;
            std::cout << "Output Frame Width: " << output_frame->width << ", Height: " << output_frame->height << std::endl;
             ...
             * @param flags 缩放算法标志
             * @param srcFilter 源图像滤波器，通常为nullptr
             * @param dstFilter 目标图像滤波器，通常为nullptr
             * @param param 可选参数，通常为nullptr
             *
             * @return 成功：指向SwsContext的指针；失败：nullptr
             */
            sws = sws_getContext(frame->width, frame->height, (AVPixelFormat)frame->format,
                                            output_frame->width, output_frame->height, (AVPixelFormat)output_frame->format,
        ...
        std::call_once(print_rgb_once_flag_, print_rgb_fomat);

        int ret = 0;
        // 当前帧的linesize与宽度匹配时，无padding
        if(frame->linesize[0] == frame->width &&
           frame->linesize[1] == frame->width / 2 &&
           frame->linesize[2] == frame->width / 2 && false) {
            ...
            // 当前帧的linesize与宽度不匹配时，即有padding的情况

            uint8_t* dest[4] = {output_frame->data[0], nullptr, nullptr, nullptr };
            int dest_linesize[4] = {output_frame->width * 3, 0, 0, 0 };

            /**
             * @brief 将slice图片数据进行scale转换，并将结果存储到目标缓冲区中。slice指的是按行排队组成的图片数据。
             *
             * @param c SwsContext上下文，即sws_getContext返回的指针
             * @param srcSlice 一个指针数组，包含源图片所有通道数据的指针
             * @param srcStride 一个指针数组，源图片每个通道对应的行间距
             * @param srcSliceY 整型，源图中slice在进行处理时吗，要处理的数据的每个行的开始位置
             * @param srcSliceH 整型，高度，即slice的行数
             * @param dst 一个指针数组，目标图片数据指针数组
             * @param dstStride 一个指针数组，目标图片每行的字节数数组
             *
             * @return 目标图片的高度
             */
            int height = sws_scale(sws, frame->data, frame->linesize, 0, frame->height, dest, dest_linesize);
            std::cout << "sws_scale output height: " << height << std::endl;
                ...
                if(ret < 0) {
                    ...
                    while(true) {
                        ...
                        if(image_count_ == 1) {
                            ...
                            std::cout << "Frame Linesize 0: " << frame->linesize[0] << ", Linesize 1: " << frame->linesize[1] << ", Linesize 2: " << frame->linesize[2] << std::endl;
                        ...
                        std::string yuv_image_name = "/frame_" + std::to_string(image_count_) + ".yuv";
...
private:
    std::once_flag print_rgb_once_flag_;
